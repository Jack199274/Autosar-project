...
#define ICU_SW_PATCH_VERSION_C              2
...
static VAR(Icu_eGlobalStateType, ICU_VAR) Icu_eGlobalState = ICU_STATE_UNINIT;
...
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidateGlobalCall(VAR(uint8, AUTOMATIC)u8ServiceId)
{
    ...
    if (ICU_STATE_UNINIT == Icu_eGlobalState)
    {
                                                           ...
                                                           )
{
    if ((Std_ReturnType)E_OK == validation)
    {
        ...
        {
            Icu_eGlobalState = ICU_STATE_UNINIT;
        }
        else
        {
            Icu_eGlobalState = ICU_STATE_IDLE;
        }
    }
}
#endif /* ICU_VALIDATE_GLOBAL_CALL == STD_ON*/
...
#if (ICU_TIMESTAMP_USES_DMA == STD_ON)
                if(ICU_NOMCLDMACHANNEL != (*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel)
                {
                    ...
                    /* @violates @ref Icu_c_REF_10 Violates MISRA 2004 Rule 11.3, Cast from pointer type and integral type.*/
                    ...
                    tcd_config[Channel].u32ssize = (uint32)ICU_DMA_SIZE_U32;
                    tcd_config[Channel].u32dsize = (uint32)ICU_DMA_SIZE_U32;
                    tcd_config[Channel].u32soff  = (uint32)0;
                    tcd_config[Channel].u32doff  = (uint32)ICU_DMA_OFFSET_U32;
                    ...
                    {
                        tcd_config[Channel].u32num_bytes = ICU_DMA_NUM_BYTES_U32;

...
[!MACRO "ICU_CHECK_CHANNEL_VS_ISR_ENABLE"!]
[!NOCODE!]
[!SELECT "IcuConfigSet"!]
[!LOOP "./IcuChannel/*"!]
    [!IF "contains(IcuHwIP,'FTM')"!][!//
        [!IF "ecu:get('Icu.IcuConfig.Icu_FTM_ModuleSingleInterrupt') = 'TRUE'"!]
            [!VAR "IsrName" = "node:value(node:ref(IcuFtmChannelRef)/../../IcuFtmModule)"!]
        [!ELSE!]
            [!VAR "IsrName" = "concat(node:value(node:ref(IcuFtmChannelRef)/../../IcuFtmModule),'_',node:value(node:ref(IcuFtmChannelRef)/IcuFtmChannel))"!]
        [!ENDIF!]
        [!IF "(node:value((../../IcuHwInterruptConfigList/*[contains(IcuIsrHwId,$IsrName)])/IcuIsrEnable) = 'false') "!]
            [!ERROR!] ISR for [!"$IsrName"!] is not enabled in HwInterruptConfigListFrom but is used in variant "[!"variant:name( )"!]", node "[!"node:name(.)"!]" [!ENDERROR!]
        [!ENDIF!]
    [!ENDIF!][!//
    [!IF "contains(IcuHwIP,'PORT')"!][!//
        [!IF "ecu:get('Icu.IcuConfig.Icu_PORT_ModuleSingleInterrupt') = 'TRUE'"!]
            [!VAR "IsrName" = "'PORT'"!]
        [!ELSE!]
            [!VAR "IsrName" = "concat(node:value(node:ref(IcuPortChannelRef)/../../IcuPortModule),'_',node:value(node:ref(IcuPortChannelRef)/IcuPortChannel))"!]
        [!ENDIF!]
        [!IF "(node:value((../../IcuHwInterruptConfigList/*[contains(IcuIsrHwId,$IsrName)])/IcuIsrEnable) = 'false') "!]
            [!ERROR!] ISR for [!"$IsrName"!] is not enabled in HwInterruptConfigListFrom but is used in variant "[!"variant:name( )"!]", node "[!"node:name(.)"!]" [!ENDERROR!]
        [!ENDIF!]
    [!ENDIF!][!//
    [!IF "contains(IcuHwIP,'LPIT')"!][!//
        [!IF "ecu:get('Icu.IcuConfig.Icu_LPIT_ModuleSingleInterrupt') = 'TRUE'"!]
            [!VAR "IsrName" = "node:value(node:ref(IcuLpitChannelRef)/../../IcuLpitModule)"!]
        [!ELSE!]
            [!VAR "IsrName" = "concat(node:value(node:ref(IcuLpitChannelRef)/../../IcuLpitModule),'_',node:value(node:ref(IcuLpitChannelRef)/IcuLpitChannel))"!]
        [!ENDIF!]
        [!IF "(node:value((../../IcuHwInterruptConfigList/*[contains(IcuIsrHwId,$IsrName)])/IcuIsrEnable) = 'false') "!]
            [!ERROR!] ISR for [!"$IsrName"!] is not enabled in HwInterruptConfigListFrom but is used in variant "[!"variant:name( )"!]", node "[!"node:name(.)"!]" [!ENDERROR!]
        [!ENDIF!]
    [!ENDIF!][!//
    [!IF "contains(IcuHwIP,'LPTMR')"!][!//
        [!VAR "IsrName" = "concat(node:value(node:ref(IcuLptmrChannelRef)/../../IcuLptmrModule),'_',node:value(node:ref(IcuLptmrChannelRef)/IcuLptmrChannel))"!]
        [!IF "(node:value((../../IcuHwInterruptConfigList/*[contains(IcuIsrHwId,$IsrName)])/IcuIsrEnable) = 'false') "!]
            [!ERROR!] ISR for [!"$IsrName"!] is not enabled in HwInterruptConfigListFrom but is used in variant "[!"variant:name( )"!]", node "[!"node:name(.)"!]" [!ENDERROR!]
        [!ENDIF!]
    [!ENDIF!][!//
    ...
    */!][!FOR "varCounter" = "0" TO "$OutNumIcuChannels"!][!/*
    ...
    /** @brief [!"node:name(.)"!] */
    {
        ...
        */!][!IF "node:ref(IcuLptmrChannelRef)/../../IcuLptmrPinSelect = 'TRGMUX_OUTPUT'"!][!/*
            */!][!WS "8"!]((Icu_Lptmr_ControlType)((Icu_Lptmr_ControlType)0U << LPTMR_CSR_TPS_SHIFT)[!/*
        */!][!ELSEIF "node:ref(IcuLptmrChannelRef)/../../IcuLptmrPinSelect = 'ALT1'"!][!/*
            */!][!WS "8"!]((Icu_Lptmr_ControlType)((Icu_Lptmr_ControlType)1U << LPTMR_CSR_TPS_SHIFT)[!/*
        */!][!ELSEIF "node:ref(IcuLptmrChannelRef)/../../IcuLptmrPinSelect = 'ALT2'"!][!/*
            */!][!WS "8"!]((Icu_Lptmr_ControlType)((Icu_Lptmr_ControlType)2U << LPTMR_CSR_TPS_SHIFT)[!/*
        */!][!ELSEIF "node:ref(IcuLptmrChannelRef)/../../IcuLptmrPinSelect = 'ALT3'"!][!/*
            */!][!WS "8"!]((Icu_Lptmr_ControlType)((Icu_Lptmr_ControlType)3U << LPTMR_CSR_TPS_SHIFT)[!/*
        */!][!ENDIF!][!/*
        */!][!IF "node:ref(IcuLptmrChannelRef)/../../PrescalerEnable = 'true'"!][!/*
            */!][!CR!][!WS "8"!]|(Icu_Lptmr_ControlType)((Icu_Lptmr_ControlType)1U << LPTMR_PSR_PBYP_SHIFT)[!/*
        */!][!ENDIF!]),[!/*
        */!][!CR!][!WS "8"!](uint8)[!"IcuDefaultStartEdge"!],[!/*
        */!][!CR!][!WS "8"!](uint8)[!"IcuMeasurementMode"!]
 ...
 */
 CONST(Icu_Lptmr_GlobalConfigurationType,ICU_CONST) Icu_Lptmr_GlobalConfiguration_[!"$VARIANT"!]=
 {
    ...
    */!][!WS "4"!]((uint16)((uint16)[!"concat(node:value('IcuLptmrPrescaler'),'_U16')"!] << LPTMR_PSR_PRESCALE_SHIFT)) |[!/*
    */!][!ENDIF!][!/*
    */!][!IF "node:value(./IcuLptmrChannelClkSrc) = 'SIRCDIV2_CLK'"!][!/*
        */!][!WS "4"!](uint16)((uint16)(0U) << LPTMR_PSR_PCS_SHIFT) |\[!CR!][!/*
    */!][!ELSEIF "node:value(./IcuLptmrChannelClkSrc) = 'LPO1K_CLK'"!][!/*
        */!][!WS "4"!](uint16)((uint16)(1U) << LPTMR_PSR_PCS_SHIFT) |\[!CR!][!/*
    */!][!ELSEIF "node:value(./IcuLptmrChannelClkSrc) = 'RTC_CLK'"!][!/*
        */!][!WS "4"!](uint16)((uint16)(2U) << LPTMR_PSR_PCS_SHIFT) |\[!CR!][!/*
    */!][!ELSEIF "node:value(./IcuLptmrChannelClkSrc) = 'PCC_LPTMR0'"!][!/*
        */!][!WS "4"!](uint16)((uint16)(3U) << LPTMR_PSR_PCS_SHIFT) |\[!CR!][!/*
    */!][!ENDIF!][!/*
    */!][!ENDLOOP!][!/*
        */!][!WS "4"!]0x0U[!CR!][!/*
*/!]};
    ...
    [!LOOP "IcuHwInterruptConfigList/*"!]
        [!IF "node:value(IcuIsrEnable)"!]
            [!IF "contains(IcuIsrHwId,$IPVType)"!]
                [!IF "(ecu:get(concat('Icu.IcuConfig.Icu_', $IPVType,'_ModuleSingleInterrupt'))='TRUE')"!]
                    [!VAR "IPV_NAME" = "node:value(IcuIsrHwId)"!]
                [!ELSE!]
    ...
    [!LOOP "IcuHwInterruptConfigList/*"!]
        [!IF "node:value(IcuIsrEnable)"!]
            [!IF "contains(IcuIsrHwId,$IPVType)"!]
                ...
                [!IF "(($IPVType = 'FTM' or $IPVType = 'LPIT') and (ecu:get(concat('Icu.IcuConfig.Icu_', $IPVType,'_ModuleSingleInterrupt'))='TRUE'))"!]
                    [!VAR "ChannelName" = "node:value(IcuIsrHwId)"!]
                [!ELSE!]
                    [!IF "($IPVType = 'FTM')"!]

...
#if (EEP_VENDOR_ID_C != ESYS_FLASHC_VENDOR_ID)
    #error "Eep.c and Reg_eSys_Ftfc.h have different vendor ids"
#endif
...
#if ((EEP_AR_RELEASE_MAJOR_VERSION_C    != STDREGMACROS_AR_RELEASE_MAJOR_VERSION) || \
     (EEP_AR_RELEASE_MINOR_VERSION_C    != STDREGMACROS_AR_RELEASE_MINOR_VERSION) \
    )
    #error "Software Version Numbers of Eep.c and StdRegMacros.h are different"
#endif
#endif

#ifndef DISABLE_MCAL_INTERMODULE_ASR_CHECK
#if ((EEP_AR_RELEASE_MAJOR_VERSION_C    != SCHM_EEP_AR_RELEASE_MAJOR_VERSION) || \
     (EEP_AR_RELEASE_MINOR_VERSION_C    != SCHM_EEP_AR_RELEASE_MINOR_VERSION) \
    )
    #error "Software Version Numbers of Eep.c and SchM_Eep.h are different"
#endif
#endif

/* Check if current file and Eep header file are of the same Software version */
#if ((EEP_AR_RELEASE_MAJOR_VERSION_C    != ESYS_FLASHC_AR_RELEASE_MAJOR_VERSION) || \
     (EEP_AR_RELEASE_MINOR_VERSION_C    != ESYS_FLASHC_AR_RELEASE_MINOR_VERSION) || \
     (EEP_AR_RELEASE_REVISION_VERSION_C != ESYS_FLASHC_AR_RELEASE_REVISION_VERSION) \
    )
    #error "Software Version Numbers of Eep.c and Reg_eSys_Ftfc.h are different"
#endif
...
/* Check if current file and Eep_InternalTypes header file are of the same Software version */
#if ((EEP_SW_MAJOR_VERSION_C != ESYS_FLASHC_SW_MAJOR_VERSION) || \
     (EEP_SW_MINOR_VERSION_C != ESYS_FLASHC_SW_MINOR_VERSION) || \
     (EEP_SW_PATCH_VERSION_C != ESYS_FLASHC_SW_PATCH_VERSION) \
    )
    #error "Software Version Numbers of Eep.c and Reg_eSys_Ftfc.h are different"
#endif
...
#if (EEP_HTCHIP == STD_ON)
VAR(Eep_LengthType, EEP_VAR) Eep_Eeprom_u32RemainingLengthAC = 0UL;
#endif
...
#if (EEP_DISABLE_CHECKING_ALIGNMENT == STD_OFF)    
...
)
{
    ...
    *  @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type  and an integral type */
    ...
    #if(STD_ON == EEP_HTCHIP)
    /* @violates @ref eep_c_REF_32 Conversions shall not be  performed between a pointer */
    /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */    
    if((0U == REG_BIT_GET8(EEPROM_FSTAT_ADDR32, EEPROM_FSTAT_RDCOLERR_U8 | EEPROM_FSTAT_ACCERR_U8 | EEPROM_FSTAT_FPVIOL_U8 | EEPROM_FSTAT_MGSTAT0_U8 | EEPROM_FSTAT_MGSTAT1_U8 | EEPROM_FSTAT_MGSTAT2_U8 | EEPROM_FSTAT_MGSTAT3_U8))
        && (0U != Eep_Eeprom_u32TimeoutCounter)
        )
    #else
    /* @violates @ref eep_c_REF_32 Conversions shall not be  performed between a pointer */
    /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */    
    if((0U == REG_BIT_GET8(EEPROM_FSTAT_ADDR32, EEPROM_FSTAT_RDCOLERR_U8 | EEPROM_FSTAT_ACCERR_U8 | EEPROM_FSTAT_FPVIOL_U8 | EEPROM_FSTAT_MGSTAT0_U8))
       && (0U != Eep_Eeprom_u32TimeoutCounter)
       )
    #endif
#else
...
#if (EEP_DISABLE_DEM_REPORT_ERROR_STATUS == STD_OFF)
    #if(STD_ON == EEP_HTCHIP)
        if(0U != REG_BIT_GET8(EEPROM_FSTAT_ADDR32,  EEPROM_FSTAT_MGSTAT1_U8 | EEPROM_FSTAT_MGSTAT2_U8 | EEPROM_FSTAT_MGSTAT3_U8))
        {
            if ((VAR(boolean, EEP_VAR))TRUE == Eep_pConfigPtr->Eep_E_MGSTAT_BlockCfg.state)
            {
                Dem_ReportErrorStatus((VAR(Dem_EventIdType, AUTOMATIC))Eep_pConfigPtr->Eep_E_MGSTAT_BlockCfg.id, DEM_EVENT_STATUS_FAILED);
            }
        }
    #endif
#endif
...
#if (EEP_HTCHIP == STD_ON)
    VAR( uint32, AUTOMATIC ) u32TempDestHT = 0UL;
    VAR( uint32, AUTOMATIC ) u32Mask = 0UL;
    VAR( volatile uint8, AUTOMATIC) u8Alignment = (uint8)0;
    VAR( uint8, AUTOMATIC) u8Alignment2 = (uint8)0;
    VAR( uint8, AUTOMATIC) u8byte = (uint8)0;
    VAR( uint8, AUTOMATIC) u8shift = (uint8)0;
#endif
...
#if (EEP_HTCHIP == STD_ON)
        if( u8PageSize == EEP_PAGE_SIZE_4BYTES)
        {
            ...
            *pu32TempDest = *pu32TempSrc;
            ...
            Eep_Eeprom_u32RemainingLength = Eep_Eeprom_u32RemainingLength - EEP_PAGE_SIZE_4BYTES;
            
            /* Update source and destination pointers. */        
            Eep_Eeprom_u32DestAddress = Eep_Eeprom_u32DestAddress + EEP_PAGE_SIZE_4BYTES;
            if(EEP_JOB_WRITE == Eep_Eeprom_eJob)
            {
                Eep_Eeprom_u32SrcAddress = Eep_Eeprom_u32SrcAddress + EEP_PAGE_SIZE_4BYTES;
            }
        }
        else if( u8PageSize != EEP_PAGE_SIZE_0BYTES)
        {
            /* The write is not 4-byte aligned. Only the needed bytes should be changed. */
            u8Alignment = Eep_Eeprom_u32DestAddress % EEP_PAGE_SIZE_4BYTES;
            pu32TempDest = (uint32 *) (Eep_Eeprom_u32DestAddress - u8Alignment);
            u32TempDestHT = *((uint32 *)pu32TempDest);
            u8Alignment2 = u8Alignment;
            
            /* Load the desired write pattern. */
            if(EEP_JOB_WRITE == Eep_Eeprom_eJob)
            {
            ...
            while((Eep_Eeprom_u32RemainingLength > 0) &&  (u8Alignment2 < EEP_PAGE_SIZE_4BYTES))
            {
                /* Calculate shift value */
                u8shift = 8 * u8Alignment2;
                /* Get the byte. */
                u8byte = (*pu8TempSrc);            
                /* Write the byte in its place. */
                u32Mask = ~(0xFF << u8shift);
                u32TempDestHT &= u32Mask;
                u32TempDestHT = u32TempDestHT + (u8byte << u8shift);
                /* Update remaining size. */
                Eep_Eeprom_u32RemainingLength = Eep_Eeprom_u32RemainingLength - EEP_PAGE_SIZE_1BYTE;
                /* Update byte place variable. */
                u8Alignment2 = u8Alignment2 + EEP_PAGE_SIZE_1BYTE;
                if(EEP_JOB_WRITE == Eep_Eeprom_eJob)
                {
                    /* Update variable */
                    pu8TempSrc = pu8TempSrc + EEP_PAGE_SIZE_1BYTE;
                }
            }
            
            *pu32TempDest = u32TempDestHT;
            
            /* Update source and destination pointers. */        
            Eep_Eeprom_u32DestAddress = Eep_Eeprom_u32DestAddress + u8Alignment2 - u8Alignment;
            if(EEP_JOB_WRITE == Eep_Eeprom_eJob)
            {
                Eep_Eeprom_u32SrcAddress = Eep_Eeprom_u32SrcAddress + u8Alignment2 - u8Alignment;
            }
        }
...
)
{
    ...
    {
        u8PageSize = Eep_Eeprom_CalculateAlignment(
                                                   u32TempSrc
                                                   #if (EEP_DISABLE_CHECKING_ALIGNMENT == STD_OFF)
                                                   , u32TempDest
                                                   #endif
                                                   , u32LocalTransferSize
                                                   );

        if(EEP_PAGE_SIZE_4BYTES == u8PageSize)
        {
            /* @violates @ref eep_c_REF_25 Conversions shall not be  performed between a pointer */
            /* @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
            ...
            u32TempSrc = u32TempSrc + EEP_PAGE_SIZE_4BYTES;
            u32TempDest = u32TempDest + EEP_PAGE_SIZE_4BYTES;
            u32LocalTransferSize = u32LocalTransferSize - EEP_PAGE_SIZE_4BYTES;
        }
        else
        {
            /* Read the entire 4 bytes, due to hardware limitation. */
            u8Alignment = u32TempSrc % EEP_PAGE_SIZE_4BYTES;
            u32TempSrc = u32TempSrc - u8Alignment;
            u32TempDestHT = *((uint32 *)u32TempSrc);
            u32TempSrc = u32TempSrc + EEP_PAGE_SIZE_4BYTES;
            /* Copy only the requested bytes. */
            while ((u8Alignment < EEP_PAGE_SIZE_4BYTES) && (u32LocalTransferSize > 0))
            {
                /* Calculate shift value */
                u8shift = 8 * u8Alignment;
                /* Get the byte. */
                u8byte = (uint8)(u32TempDestHT >> u8shift);
                /*  */
                if ( u8byte != *((uint8 *)u32TempDest))
                {
                    SchM_Exit_Eep_EEP_EXCLUSIVE_AREA_09();
                    /* @violates @ref eep_c_REF_5 Return statement before end of function. */
                    return EEPROM_E_BLOCK_INCONSISTENT;
                ...
                u32TempDest = u32TempDest + EEP_PAGE_SIZE_1BYTE;
                u8Alignment = u8Alignment + EEP_PAGE_SIZE_1BYTE;
            }
        }
    }
#else /* #if (EEP_HTCHIP == STD_ON) */
    while (u32LocalTransferSize > 0UL)
    {
        u8PageSize = Eep_Eeprom_CalculateAlignment(
                                                   u32TempSrc
...
)
{
...
#if (EEP_HTCHIP == STD_ON)
    Eep_Eeprom_u32RemainingLengthAC = u32TransferSize;
#endif
    ...
    {
        u8PageSize = Eep_Eeprom_CalculateAlignment(
                                                   Eep_Eeprom_u32DestAddress
                                                   #if (EEP_DISABLE_CHECKING_ALIGNMENT == STD_OFF)
                                                   , Eep_Eeprom_u32SrcAddress
                                                   #endif
                                                   , u32LocalTransferSize
                                                   );
...
#if (EEP_HTCHIP == STD_ON)
        /* On High Temperature chips the remaining size is updated in the access code. */
        u32LocalTransferSize =  Eep_Eeprom_u32RemainingLengthAC;
#else
        u32LocalTransferSize = u32LocalTransferSize - u8PageSize;
#endif
...
)
{
    ...
    /* @violates @ref eep_c_REF_32 Conversions shall not be  performed between a pointer
    * @violates @ref eep_c_REF_8 A cast should not be performed between a pointer type */
    if((u32TargetAddress > (Eep_AddressType)EEP_TOTAL_SIZE ) || \
      ((u32TargetAddress % EEP_PAGE_SIZE_4BYTES) != 0U) || \

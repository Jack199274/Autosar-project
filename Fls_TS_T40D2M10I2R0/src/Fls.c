...
#define FLS_SW_PATCH_VERSION_C               2
...
/* Check if current file and Fls_Flash.h header file are of the same Autosar version */
    ...
    #if ((FLS_AR_RELEASE_MAJOR_VERSION_C != MCL_AR_RELEASE_MAJOR_VERSION_H) || \
         (FLS_AR_RELEASE_MINOR_VERSION_C != MCL_AR_RELEASE_MINOR_VERSION_H) \
        )
...
*/
VAR( Fls_SectorIndexType, FLS_VAR) Fls_u32JobSectorItPos = 0UL;
/**
...
* This variable maybe used in LLD layer
*/
VAR( Fls_SectorIndexType, FLS_VAR) Fls_u32JobSectorEndPos = 0UL;
...
{
    VAR( boolean, AUTOMATIC ) bRetVal = (boolean)FALSE;
    VAR( Fls_SectorIndexType, AUTOMATIC ) u32SectorIndex = 0U; 
    VAR( Fls_AddressType, AUTOMATIC ) u32OffsetAddr = 0U;

    u32SectorIndex = Fls_GetSectorIndexByAddr( u32TargetAddress );
    u32OffsetAddr = u32TargetAddress - (*(Fls_pConfigPtr->paSectorStartAddr))[u32SectorIndex] ;
    /*
    ...
    */
    if( ( u32TargetAddress < FLS_TOTAL_SIZE ) &&
        ( (u32OffsetAddr % (*(Fls_pConfigPtr->paSectorSize))[u32SectorIndex]) == 0U)
      )
    {
        bRetVal = (boolean)TRUE;
    }
...
( CONST(Fls_AddressType, AUTOMATIC) u32TargetAddress )
{
    ...
    u32SectorIndex = Fls_GetSectorIndexByAddr( u32TargetAddress );
    u32OffsetAddr = (u32TargetAddress + 1U) - (*(Fls_pConfigPtr->paSectorStartAddr))[u32SectorIndex] ;
...
static FUNC( MemIf_JobResultType, FLS_CODE )  Fls_DoJobErase( void )
{
    ...
    */
    ...
    else
    {
         ...
         * evaluation that the standard permits.
         */
        if(( FLASH_E_OK == eLldRetVal ) && (Fls_u32JobSectorItPos == Fls_u32JobSectorEndPos) && ( Fls_u32JobSectorIt == Fls_u32JobSectorEnd ))
        {
            eRetVal = MEMIF_JOB_OK;
        }
...
)
{
    ...
    VAR( Fls_SectorIndexType, AUTOMATIC ) u32TmpJobSectorItPos = 0UL;
    VAR( Fls_SectorIndexType, AUTOMATIC ) u32TmpJobSectorEndPos = 0UL;
...
#endif    /* FLS_DEV_ERROR_DETECT == STD_ON */
    {
        ...
        u32TmpJobSectorItPos = (u32TargetAddress - (*(Fls_pConfigPtr->paSectorStartAddr))[u32TmpJobSectorIt]) /
                               (*(Fls_pConfigPtr->paSectorSize))[u32TmpJobSectorIt];
        u32TmpJobSectorEndPos = ((u32TargetAddress + u32Length) - ((*(Fls_pConfigPtr->paSectorStartAddr))[u32TmpJobSectorEnd] + (*(Fls_pConfigPtr->paSectorSize))[u32TmpJobSectorEnd])) /
                               (*(Fls_pConfigPtr->paSectorSize))[u32TmpJobSectorEnd];
        ...
        else
        {
            ...
            Fls_u32JobSectorItPos = u32TmpJobSectorItPos;
            Fls_u32JobSectorEndPos = u32TmpJobSectorEndPos;

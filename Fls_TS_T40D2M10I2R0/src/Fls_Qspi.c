...
#define FLS_QSPI_SW_PATCH_VERSION_C                    2
...
/* Check if current file and Fls_Qspi_Types.h header file are of the same Autosar version */
    ...
    #if ((FLS_QSPI_AR_RELEASE_MAJOR_VERSION_C != MCL_AR_RELEASE_MAJOR_VERSION_H) || \
         (FLS_QSPI_AR_RELEASE_MINOR_VERSION_C != MCL_AR_RELEASE_MINOR_VERSION_H) \
        )
...
FUNC( void, FLS_CODE ) Fls_Qspi_Init( void )
{
    ...
    for( u8Iter = 0U; u8Iter < Fls_pConfigPtr->pFlsQspiCfgConfig->u8QspiUnitsCount; u8Iter++ )
    {
        ...
        REG_RMW32( u32ExtHwUnitBaseAddr + QSPI_SFACR_OFFSET_U32, QSPI_SFACR_WA_U32, \
                   QSPI_SFACR_WA_U32 & ((uint32)((uint32)((*(Fls_pConfigPtr->pFlsQspiCfgConfig->paQspiUnitCfg))[u8Iter].bQspiUnitWordAddressable) << QSPI_SFACR_WA_SHIFT_U32)) \
                 );
        /* Populate LUT. */
...
)
{
    ...
    /* If no. of bytes is not divisible by 4, round to next integer to expect 1 more word. Ex: 5 bytes -> 2 words -> WMRK=1. */
    REG_RMW32( Fls_Qspi_u32HwUnitBaseAddr +  QSPI_RBCT_OFFSET_U32, QSPI_RBCT_WMRK_MASK_U32, (uint32)(( ((uint32)u8BytesToRead + 3U) / 4U ) - 1U) );
...
)
{
    ...
    do
    {
        ...
        if( (0U == (u32StartAddr % 4U)) && (0U == ((uint32)p8DataReadPtr % 4U)) && (4U <= u32DataLength) )  /* An aligned entry(32bits, 4 bytes) and at least 4bytes have to be read. */
        {
            ...
            {
                VAR(uint32, AUTOMATIC) u32ApplData = FLS_ERASED_VALUE;
                ...
                /* Compare or Write Verify or Blank Check operation type). */
                /* Compiler_Warning: see fls_c_REF_CW_02 */
                if( u32ApplData != *((volatile uint32 *)u32StartAddr) )
                {
        ...
        else /* if( (0U != (u32StartAddr % 4U)) || (4U > u32DataLength) ), read unaligned or byte entries. */
        {
            ...
            {
                VAR(uint8, AUTOMATIC) u8ApplData = (uint8)FLS_ERASED_VALUE;

                if( NULL_PTR != p8DataCmpPtr )
                {
                    /* @violates @ref fls_qspi_c_REF_6 A cast should not be performed between a pointer to object type and a different pointer to object type. */
                    u8ApplData = *((volatile const uint8 *)p8DataCmpPtr);
                ...
                /* Compare or Write Verify or Blank Check operation type). */
                /* Compiler_Warning: see fls_c_REF_CW_02 */
                if( u8ApplData != *((volatile uint8 *)u32StartAddr) )
                {
                    /* Error, data read does not match the provided compare buffer.*/
                    /*  @violates @ref fls_qspi_c_REF_4 Return statement before end of function. */
                    return FLASH_E_BLOCK_INCONSISTENT;
...
)
{
    ...
    do
    {
        /* Read aligned word entries if they exist. */
        /* Compiler_Warning: see fls_c_REF_CW_02 */
        if ( (0U == ((uint32)p8DataReadPtr % 4U)) && (0U == ((uint32)p8DataCmpPtr % 4U)) && (0U == ((u32RXBuffBaseAddr + (uint32)u8Iter) % 4U)) && (4U <= u8BytesToRead) )  /* An aligned entry(32bits, 4 bytes) and at least 4bytes have to be read. */
        {
            /* Check what operation is intended: read, compare, blank check or write verify. */
            if( NULL_PTR != p8DataReadPtr)
            {
...
)
{
    ...
    VAR(uint32, AUTOMATIC) u32SectorSize = 0UL;
#endif
    VAR(uint32, AUTOMATIC) u32OffsetAddress = 0U;
    u32OffsetAddress = Fls_u32JobSectorItPos*((*(Fls_pConfigPtr->paSectorSize))[Fls_u32JobSectorIt]);
    /* Store the address of the sector start, at which the erase is executed. */
    Fls_Qspi_u32MemoryAddress =  Fls_Qspi_u32ExtSectStartAddr + u32OffsetAddress;
    ...
    #if( FLS_ERASE_BLANK_CHECK == STD_ON )
        /* Sector size to be erase */
        u32SectorSize = (*(Fls_pConfigPtr->paSectorSize))[Fls_u32JobSectorIt];
        /* Check if memory locations were successfully erased. */
        if (FLASH_E_OK != Fls_Qspi_VerifyErase(Fls_Qspi_u32MemoryAddress, u32SectorSize))
        {
    ...
    else /* Asynchronous mode(ASYNC or IRQ mode). */
    {
        ...
        Fls_u32LLDNumberOfStepBack = (*(Fls_pConfigPtr->paSectorSize))[Fls_u32JobSectorIt];
        #endif
...
FUNC( void, FLS_CODE ) Fls_Qspi_MainFunctionErase( void )
{
        ...
        (FLASH_E_OK == eExtMemIdle) )          /* Poll the external memory BUSY bit. */
    {
        ...
        else /* Erase operation was successful */
        {
            ...
            Fls_u32JobSectorItPos++; /* The sector was successfully erased, increment the sector iterator.*/
            if(Fls_u32JobSectorIt < Fls_u32JobSectorEnd)
            {
                if(Fls_u32JobSectorItPos == (*(Fls_pConfigPtr->paNumberOfSector))[Fls_u32JobSectorIt])
                {
                    Fls_u32JobSectorIt++;
                    Fls_u32JobSectorItPos = 0U;
                }
            }
            else
            {
                if(Fls_u32JobSectorItPos > Fls_u32JobSectorEndPos)
                {
                    Fls_u32JobSectorIt++;
                }
            }
...
FUNC( void, FLS_CODE ) Fls_Qspi_MainFunctionEraseBlankCheck( void )
{
    ...
    else /* Fls_u32LLDNumberOfStepBack <= FLS_MAX_ERASE_BLANK_CHECK  - only this MainFunction call is needed. */
    {
        ...
        Fls_u32JobSectorItPos++;   /* The sector was successfully erased, increment the sector pos iterator.*/
        if(Fls_u32JobSectorIt < Fls_u32JobSectorEnd)
        {
            if(Fls_u32JobSectorItPos == (*(Fls_pConfigPtr->paNumberOfSector))[Fls_u32JobSectorIt])
            {
...
)
{
    ...
    {
        /* Check if u32SourceAddr is unaligned. */
        if (0U !=  (u32SourceAddr % 4U))
        {
            VAR(uint8, AUTOMATIC) u8Iter2 = 0U;
            VAR(uint32, AUTOMATIC) u32TxIncompleteWord = 0UL;
            /* Loop through 4 unaligned bytes. */
            for(u8Iter2=0U; u8Iter2 < 4U; u8Iter2++)
            {
                VAR(uint8, AUTOMATIC) u8TmpData = 0U;
                u8TmpData = (*((volatile uint8 *)u32SourceAddr));
                u32TxIncompleteWord = u32TxIncompleteWord | (uint32)((uint32)u8TmpData << ((uint32)u8Iter2*8UL)); /* Write 1 byte. */
...
)
{
    ...
    if( (0U != (QSPI_FR_RBDF_U32 & u32FlagsReg)) && (0U != (QSPI_RSER_RBDIE_U32 & u32EnableReg)) )
    {
        ...
        if(FLASH_E_OK == eRetVal)
        {
            ...
            /* Clear the interrupt flag(w1c) - the RBDF flag has to be read only after reading the values from RX buffer, because clearing it will pop/discard a number of WMRK values. */
            REG_BIT_SET32(Fls_Qspi_u32HwUnitBaseAddr + QSPI_FR_OFFSET_U32, QSPI_FR_RBDF_U32);
            ...
            u8QspiUnitBusyBitPosOff = ((*(Fls_pConfigPtr->pFlsQspiCfgConfig->paQspiUnitCfg))[Fls_Qspi_u8HwUnitCfgIndex].u8QspiUnitBusyBitPosOffset);
            /*  @violates @ref fls_qspi_c_REF_7 Cast of complex expression changes signedness. */
            /* Check if the Busy bit from the status register has the value corresponding to the busy state of the memory. bMemoryIsBusy = (boolean)( u32StatusRegVal == (u8QspiUnitBusyBitValue << u8QspiUnitBusyBitPosOffset) ); */
            *pbMemoryIsBusy = (boolean)( Fls_Qspi_u32StatusRegVal == (uint32)((uint32)u8QspiUnitBusyBitVal << (uint32)u8QspiUnitBusyBitPosOff) );
...
)
{
    ...
    if( (0U != (QSPI_FR_RBDF_U32 & u32FlagsReg)) && (0U != (QSPI_RSER_RBDIE_U32 & u32EnableReg)) )
    {
        ...
        if(FLASH_E_OK == eRetVal)
        {
            ...
            u8QspiUnitBusyBitPosOff = ((*(Fls_pConfigPtr->pFlsQspiCfgConfig->paQspiUnitCfg))[Fls_Qspi_u8HwUnitCfgIndex].u8QspiUnitBusyBitPosOffset);
            /*  @violates @ref fls_qspi_c_REF_7 Cast of complex expression changes signedness. */
            /* Check if the Busy bit from the status register has the value corresponding to the busy state of the memory. bMemoryIsBusyPar = (boolean)( u32StatusRegVal == (u8QspiUnitBusyBitValue << u8QspiUnitBusyBitPosOffset) ); */
            *pbMemoryIsBusyPar = (boolean)( Fls_Qspi_u32StatusRegVal == (uint32)((uint32)u8QspiUnitBusyBitVal << (uint32)u8QspiUnitBusyBitPosOff) );
        ...
        if(FLASH_E_OK == eRetVal)
        {
            ...
            if(0U < Fls_u32LLDRemainingDataToRead)
            {
                ...
                u8LLDBytesToRead = Fls_Qspi_u8LLDBytesToRead;  /* Intermediate variable used to avoid order of evaluation issue caused by volatile specifier. */
                 /* If no. of bytes is not divisible by 4, round to next integer to expect 1 more word. Ex: 5 bytes -> 2 words -> WMRK=1. */
                REG_RMW32( Fls_Qspi_u32HwUnitBaseAddr +  QSPI_RBCT_OFFSET_U32, QSPI_RBCT_WMRK_MASK_U32, (uint32)(( ((uint32)u8LLDBytesToRead + 3U) / 4U ) - 1U) );

                /* Enable the Receive Buffer Drain Interrupt. */
                REG_BIT_SET32(Fls_Qspi_u32HwUnitBaseAddr + QSPI_RSER_OFFSET_U32, QSPI_RSER_RBDIE_U32);

                /* Re-launch the last LUT command, changing only the number of bytes.(A LUT command is triggered whenever the SEQID field is written,
                so by using a RMW operation, the previous LUT sequence ID which which is already written in the register is reused and only the DATASZ field is updated) */
                u8LLDBytesToRead = Fls_Qspi_u8LLDBytesToRead;
    ...
    if( (0U != (QSPI_FR_RBDF_U32 & u32FlagsReg)) && (0U != (QSPI_RSER_RBDIE_U32 & u32EnableReg)) )
    {
        ...
        if(FLASH_E_OK == eRetVal)
        {
            ...
            if(0U < Fls_u32LLDRemainingDataToRead)
            {
                ...
                u8LLDBytesToRead = Fls_Qspi_u8LLDBytesToRead;       /* Use an intermediary variable in order to avoid volatile order of evaluation side effects compiler warning. */
                 /* If no. of bytes is not divisible by 4, round to next integer to expect 1 more word. Ex: 5 bytes -> 2 words -> WMRK=1. */
                REG_RMW32( Fls_Qspi_u32HwUnitBaseAddr +  QSPI_RBCT_OFFSET_U32, QSPI_RBCT_WMRK_MASK_U32, ( ((uint32)u8LLDBytesToRead + 3U) / 4U ) - 1U );
...
FUNC(Fls_LLDReturnType, FLS_CODE) Fls_Qspi_IrqWriteBlankCheck(void)
{
    ...
    if( (0U != (QSPI_FR_RBDF_U32 & u32FlagsReg)) && (0U != (QSPI_RSER_RBDIE_U32 & u32EnableReg)) )
    {
        /* Read the expected number of bytes from the RX buffer (Fls_Qspi_u8LLDBytesToRead is set previously in the Fls_Qspi_VerifyErase() function,
        when scheduling the read job). The Fls_Qspi_ReadRXBuffer() functions checks internally the read values against the erase value. */
        eRetVal = Fls_Qspi_ReadRXBuffer(Fls_Qspi_u8LLDBytesToRead, NULL_PTR, NULL_PTR);
        if(FLASH_E_OK == eRetVal)
        {
            VAR(uint8, AUTOMATIC) u8LLDBytesToRead = 0U;
            ...
            if(0U < Fls_u32LLDRemainingDataToRead)
            {
                ...
                /* Re-launch the last LUT command, changing only the number of bytes.(A LUT command is triggered whenever the SEQID field is written, so by using a RMW operation,
                the previous LUT sequence ID which which is already written in the register is reused and only the DATASZ field is updated) */
                REG_RMW32( Fls_Qspi_u32HwUnitBaseAddr + QSPI_IPCR_OFFSET_U32, QSPI_IPCR_IDATSZ_MASK_U32, (uint32)((uint32)u8LLDBytesToRead << QSPI_IPCR_IDATSZ_SHIFT_U32) );
...
FUNC(void, FLS_CODE) Fls_Qspi_SetRXWmrkExt(CONST(Fls_QspiUnitNameType, AUTOMATIC) eHwUnitName, CONST(uint8, AUTOMATIC) u8BytesToRead)
{
    ...
    /* Set the WMRK value. */
    /* If no. of bytes is not divisible by 4, round to next integer to expect 1 more word. Ex: 5 bytes -> 2 words -> WMRK=1. */
    REG_RMW32( u32HwUnitBaseAddr + QSPI_RBCT_OFFSET_U32, QSPI_RBCT_WMRK_MASK_U32, (uint32)( (((uint32)u8BytesToRead + 3UL) / 4UL) - 1UL ) );
}
...
)
{
    ...
    (void)u8CoarseDelay;    /* The coarse delay parameter is not configurable on SOC implementation. */

...
[!IF "var:defined('postBuildVariant')"!][!//
    ...
    [!//
        ...
        [!CODE!][!WS4!]extern CONST(Fls_ConfigType, FLS_CONST) Fls_Config_[!"."!];[!CR!][!ENDCODE!]
    [!ENDLOOP!][!//
[!ELSE!][!//
    [!CODE!][!WS4!]/* @violates @ref Fls_Cfg_H_REF_2 Could define variable at block scope */[!CR!][!ENDCODE!]
    [!CODE!][!WS4!]extern CONST(Fls_ConfigType, FLS_CONST) Fls_Config;[!CR!][!ENDCODE!]
[!ENDIF!][!//
[!ENDNOCODE!][!//
[!ENDMACRO!]
/*==================================================================================================
...
#define FLS_TRANSIENT_FAULT_DETECT  ([!IF "FlsGeneral/FlsTransientFaultDetect"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
#define FLS_WRITE_VERIFY_CHECK      ([!IF "FlsGeneral/FlsWriteVerificationEnabled"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/* Pre-processor switch to enable / disable the erase blank check */
#define FLS_ERASE_BLANK_CHECK       ([!IF "FlsGeneral/FlsEraseVerificationEnabled"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
#define FLS_TIMEOUT_HANDLING        ([!IF "FlsGeneral/FlsTimeoutSupervisionEnabled"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

[!IF "FlsGeneral/FlsTimeoutSupervisionEnabled"!]
/* Timeout value for Erase and Write operation - internal flash operations. */
#define FLS_ASYNC_WRITE_TIMEOUT_VALUE       ([!"FlsGeneral/FlsAsyncWriteTimeout"!]U)
#define FLS_ASYNC_ERASE_TIMEOUT_VALUE       ([!"FlsGeneral/FlsAsyncEraseTimeout"!]U)
#define FLS_SYNC_WRITE_TIMEOUT_VALUE        ([!"FlsGeneral/FlsSyncWriteTimeout"!]U)
#define FLS_SYNC_ERASE_TIMEOUT_VALUE        ([!"FlsGeneral/FlsSyncEraseTimeout"!]U)
/* Timeout value for Erase and Write operation - QSPI flash operations. */
#define FLS_QSPI_SYNC_READ_TIMEOUT_VALUE    ([!"FlsGeneral/FlsQspiSyncReadTimeout"!]U)
#define FLS_QSPI_SYNC_WRITE_TIMEOUT_VALUE   ([!"FlsGeneral/FlsQspiSyncWriteTimeout"!]U)
#define FLS_QSPI_SYNC_ERASE_TIMEOUT_VALUE   ([!"FlsGeneral/FlsQspiSyncEraseTimeout"!]U)
#define FLS_QSPI_ASYNC_WRITE_TIMEOUT_VALUE  ([!"FlsGeneral/FlsQspiAsyncWriteTimeout"!]U)
#define FLS_QSPI_ASYNC_ERASE_TIMEOUT_VALUE  ([!"FlsGeneral/FlsQspiAsyncEraseTimeout"!]U)
#define FLS_QSPI_IRQ_READ_TIMEOUT_VALUE     ([!"FlsGeneral/FlsQspiIrqReadTimeout"!]U)
#define FLS_QSPI_IRQ_ERASE_TIMEOUT_VALUE    ([!"FlsGeneral/FlsQspiIrqEraseTimeout"!]U)
#define FLS_QSPI_IRQ_WRITE_TIMEOUT_VALUE    ([!"FlsGeneral/FlsQspiIrqWriteTimeout"!]U)
...
/* Timeout value for Erase and Write operation - FLEXSPI flash operations. */
#define FLS_FLEXSPI_SYNC_READ_TIMEOUT_VALUE    (0U)
#define FLS_FLEXSPI_SYNC_WRITE_TIMEOUT_VALUE   (0U)
#define FLS_FLEXSPI_SYNC_ERASE_TIMEOUT_VALUE   (0U)
#define FLS_FLEXSPI_ASYNC_WRITE_TIMEOUT_VALUE  (0U)
#define FLS_FLEXSPI_ASYNC_ERASE_TIMEOUT_VALUE  (0U)
#define FLS_FLEXSPI_IRQ_READ_TIMEOUT_VALUE     (0U)
#define FLS_FLEXSPI_IRQ_ERASE_TIMEOUT_VALUE    (0U)
#define FLS_FLEXSPI_IRQ_WRITE_TIMEOUT_VALUE    (0U)
/* Timeout value for Abort operation */
...
[!IF "node:exists(AutosarExt/FlsCheckFlexNvmRatio)"!][!//
/* Flash check FlexNvm partion ratio at initialization */
#define FLASH_CHECK_FLEXNVM_RATIO           ([!IF "AutosarExt/FlsCheckFlexNvmRatio"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
[!ENDIF!][!//
...
[!IF "node:exists(AutosarExt/FlsExtendedReadErrorCheck)"!][!//
/* Flash enable redundant read error check */
#define FLS_EXTENDED_READ_ERROR_CHECK           ([!IF "AutosarExt/FlsExtendedReadErrorCheck"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
[!ENDIF!][!//

[!IF "ecu:get('Fls.UserModeSupport')='true'"!] [!//
/* User mode support */
        #define FLS_ENABLE_USER_MODE_SUPPORT        ([!IF "AutosarExt/FlsEnableUserModeSupport"!]STD_ON [!ELSE!] STD_OFF [!ENDIF!])
...
[!ENDIF!] [!//
        ...
        [!IF "node:exists(AutosarExt/FlsSynchronizeCache)"!][!//
        #define FLS_SYNCRONIZE_CACHE    [!WS "4"!]([!IF "AutosarExt/FlsSynchronizeCache"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
        [!ELSE!][!//
...
#define FLS_DATA_FLASH_EXISTS         ([!IF "ecu:get('Fls.DataFlashExists')='STD_ON'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
[!IF "ecu:has('Fls.EccSyndromeAndDataAddressCheck')"!][!//
    [!IF "ecu:get('Fls.EccSyndromeAndDataAddressCheck') = 'true'"!][!//
    ...
    [!ELSE!] [!//
        /* In  Fls_DsiHandler function will not additionally check syndrome and data address */
        #define FLS_ECC_SYNDROME_AND_DATA_ADDRESS_CHECK  (STD_OFF)
    [!ENDIF!] [!//
[!ELSE!] [!//
    /* In  Fls_DsiHandler function will not additionally check syndrome and data address */
    #define FLS_ECC_SYNDROME_AND_DATA_ADDRESS_CHECK  (STD_OFF)
...
#define FLS_QSPI_LOCK_LUT                   ([!IF "AutosarExt/FlsQspiLockLUT"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
/* Fls internal hardware unit */
#define FLS_FTFM                            ([!IF "ecu:get('Fls.Internal.HwUnit') = 'FLS_FTFM'"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
/*==================================================================================================
...
typedef struct
{
    ...
    */
    CONST( Fls_AddressType, FLS_CONST) (*paSectorSize)[]; 
      /** 
    * @brief pointer to array containing number of hw sector 
    */
    CONST( uint16, FLS_CONST) (*paNumberOfSector)[];
    /**

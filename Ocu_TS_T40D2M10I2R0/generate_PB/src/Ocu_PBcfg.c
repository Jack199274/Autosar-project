...
/* Macro used to generate the correspondence (translation) table between all Ftm available 
channels and the indexes of channels in the array of Ocu channels configured to use Ftm */
[!MACRO "GenerateeHwToLogicMap", "nameIpv", "number_channel", "numberHWmodule"!]
[!FOR "idx" = "0" TO "$numberHWmodule - 1"!]
    [!NOCODE!]
    [!VAR "OutVareTimerHwToLogicArray" = "''"!]
    [!VAR "OutVareTimerHwToLogicComment" = "''"!]   
    /* Loop through all channels of eTimer module received as parameter */
    [!FOR "MacCurModChannel" = "0" TO "num:i($number_channel)"!]
        /* Limit max number of configured channels to 255 */
        [!VAR "MacVarChTranslationIndex" = "255"!]

        /* Loop through all Ocu channels configured to use eTimer IP */
        [!LOOP "OcuChannel/*"!]
            [!IF "(contains(FtmHwChannel, $nameIpv))"!]
                /* Build the comment for the current Module */
                [!VAR "OutVareTimerHwToLogicComment" = "concat('/* Idx of channels 0 - ', num:i(7), ' of eTimer Module ', num:i($idx), ' in the array of logic Ocu channels */')"!]
                [!VAR "MacOcuAssignedHardwareChannel" = "OcuAssignedHardwareChannel"!]
                [!VAR "MacOcuChannelId" = "OcuChannelId"!]
               
                
                [!VAR "MaceTimerModule"     = "num:i(substring-before(substring-after(FtmHwChannel,concat( $nameIpv ,'_')),'_CH_'))"!]
                [!VAR "MaceTimerChannel" = "num:i(substring-after(FtmHwChannel,'_CH_'))"!]    
                /* Check the match with the module received as parameter */
                [!IF "$MaceTimerModule = $idx"!]
                    /* Check the match with the channel loop var */
                    [!IF "num:i($MaceTimerChannel) = $MacCurModChannel"!] 
                        /* Keep the index of the channel in the array of configured Ocu channels */
                        [!VAR "MacVarChTranslationIndex" = "node:name(.)"!]
                        [!VAR "OcuHardwareChannel_id" = "FtmHwChannel"!]
                    [!ENDIF!]
                [!ENDIF!]
            [!ENDIF!]   
        [!ENDLOOP!]

        /* If no configured Ocu channel is using the current eTimer channel in Module  */
        [!IF "$MacVarChTranslationIndex = 255"!]
            [!CODE!]
            (Ocu_ChannelType)255,          /* [!"$nameIpv"!]_[!"$idx"!]_CH_[!"$MacCurModChannel"!] */
            [!ENDCODE!]
        [!ELSE!]
            [!CODE!]
            (Ocu_ChannelType)[!"$MacVarChTranslationIndex"!], /* [!"$OcuHardwareChannel_id"!] */
            [!ENDCODE!]
        [!ENDIF!]
...
#define OCU[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBCFG_MODULE_ID_C                      125
#define OCU[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBCFG_AR_RELEASE_MAJOR_VERSION_C       4
#define OCU[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBCFG_AR_RELEASE_MINOR_VERSION_C       3
#define OCU[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBCFG_AR_RELEASE_REVISION_VERSION_C    1
...
#define OCU[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBCFG_SW_PATCH_VERSION_C               2
/**@}*/
...
static CONST(Ocu_ChannelType, OCU_CONST)  Ocu_InitHWMap_PB[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!][OCU_HW_CHANNELS_NO]=
    ...
    {
[!CALL "GenerateeHwToLogicMap", "nameIpv" = "'FTM'", "number_channel" = "'7'", "numberHWmodule" = "$numFtmModules"!]
    };
#endif
[!ENDCODE!]
...
* @violates @ref Ocu[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBcfg_C_REF_2 external ... could be made static
*/
CONST(Ocu_ConfigType, OCU_CONST) Ocu_Config =
    [!ENDCODE!]
[!ELSE!]
    [!CODE!]
/**
...
* @violates @ref Ocu[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!]_PBcfg_C_REF_2 external ... could be made static
*/
CONST(Ocu_ConfigType, OCU_CONST) Ocu_Config[!IF "var:defined('postBuildVariant')"!]_[!"$postBuildVariant"!][!ENDIF!] =
    [!ENDCODE!]
[!ENDIF!]
[!CODE!]
{

...
#define OCU_SW_PATCH_VERSION_C                  2
/**@}*/
...
*/
VAR(Ocu_ChannelStatusType, OCU_VAR) Ocu_aChannelState[OCU_MAX_CHANNELS];
...
extern CONST(Ocu_ConfigType, OCU_CONST) Ocu_Config;
#endif
...
)
{
    ...
    if (OCU_STATUS_RUNNING == Ocu_aChannelState[ChannelNumber])
    {
    ...
    if ((OCU_STOPCHANNEL_ID == u8ServiceId) && (OCU_STATUS_STOPPED == Ocu_aChannelState[ChannelNumber]))
    {
        nRetVal = (Std_ReturnType)E_NOT_OK;        
    }
...
)
{
    /* Variable to store the value returned by the function */ 
    VAR(Std_ReturnType, AUTOMATIC) nRetVal = (Std_ReturnType)E_OK;
    VAR(Ocu_NotifyType, AUTOMATIC) Ocu_pChannelNotification;

    Ocu_pChannelNotification = (*Ocu_pConfig->pOcuChannelsConfig)[ChannelNumber].pfOcuChannelNotification;
    /* Check if Notification param is not NULL */
    if(NULL_PTR == Ocu_pChannelNotification)
    {
        (void)Det_ReportError
        (
            (uint16) OCU_MODULE_ID,
    ...
    if((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if((Std_ReturnType)E_OK == CallIsValid)
        {
            ...
            for(ChannelNumber = 0U; ChannelNumber < OCU_MAX_CHANNELS; ChannelNumber++)
            {
                ...
                Ocu_aChannelState[ChannelNumber] = OCU_STATUS_INITIALIZED;
...
)
{
    ...
    if((Std_ReturnType)E_OK == CallIsValid)
    {
        ...
        if((Std_ReturnType)E_OK == CallIsValid)
        {
            ...
            {
                /* initialize global data */  
                Ocu_aChannelState[ChannelNumber] = OCU_STATUS_UNINITIALIZED;

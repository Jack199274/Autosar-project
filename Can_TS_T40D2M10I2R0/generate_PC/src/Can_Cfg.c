                        ...
                        [!LOOP "CanIcomRxMessageSignalConfig/*"!] 
                            {
                                ...
                                [!IF "node:exists(CanIcomSignalRef)"!]
                                    1U /* [!"node:value(CanIcomSignalRef)"!] reference it is not availabel for now. It may be referenced from an upper layer*/
                                [!ELSE!]    
                                    0U
                                [!ENDIF!]
                                [!ENDINDENT!]
        ...
        [!LOOP "node:order(CanController/*,'node:value(CanControllerId)')"!]
            /* ControlerDescriptor of CanController_[!"node:value(CanControllerId)"!] */
            {
                ...
                [!IF "CanControllerActivation = 'true'"!]
                    ...
                    [!IF "(CanErrorControllerNotifEn = 'true') and (normalize-space(CanErrorFDControllerNotification) != 'NULL_PTR')"!]
                        &[!"normalize-space(CanErrorFDControllerNotification)"!],
                    [!ELSE!]
                        NULL_PTR,
                    [!ENDIF!]
                    #endif

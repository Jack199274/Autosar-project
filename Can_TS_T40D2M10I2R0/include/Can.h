...
#if ((CAN_AR_RELEASE_MAJOR_VERSION != CAN_GENERALTYPES_AR_RELEASE_MAJOR_VERSION) || \
     (CAN_AR_RELEASE_MINOR_VERSION != CAN_GENERALTYPES_AR_RELEASE_MINOR_VERSION) )
  #error "Software Version Numbers of Can.h and Can_GeneralTypes.h are different"
#endif
#endif
...
typedef struct
    {
        ...
        CONST(uint32, CAN_CONST) u32CanControllerTrcvDelayCompensation; /**< @brief Prescaler_alt written to TDCOFF register. */
#if (CAN_DUAL_CLOCK_MODE == STD_ON)
        CONST(uint32, CAN_CONST) u32CanControllerTrcvDelayCompensationAlt; /**< @brief Prescaler_alt written to TDCOFF register. */
#endif
...
typedef struct
    {
        ...
        CONST(uint16, CAN_CONST) u16CtrlPrescaler; /**< @brief Prescaler written to CTRL1 register. */
#if (CAN_DUAL_CLOCK_MODE == STD_ON)
    #ifndef CAN_MSCAN_SPECIFIC
        CONST(uint16, CAN_CONST) u16CtrlPrescalerAlt; /**< @brief Prescaler_alt written to CTRL1 register. */
    #endif
#endif
...
typedef struct
    {
        ...
        CONST(uint16, CAN_CONST) Can_u16RxFiFoHrhFirst; /**< @brief the HRH used for RX FIFO. */
...
typedef struct
    ...
    #if (CAN_FD_MODE_ENABLE == STD_ON)
        CONST(Can_PCallBackType, CAN_CONST) Can_ErrorFDNotification; /*< @brief Pointer to Error interrupt notification function (ESR[ERRFAST_INT]). */
    #endif
...
extern FUNC(void, CAN_CODE) Can_SetClockMode(VAR(Can_ClockModeType, AUTOMATIC) can_clk_mode);

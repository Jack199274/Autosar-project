...
#define CAN_FLEXCAN_SW_PATCH_VERSION_C             2
...
/* Check if current file and DET header file are of the same version */
   ...
   /* Check if current file and Mcal.h file are of the same Software version */
    ...
    #endif
   /* Check if current file and SchM_Can.h file are of the same Software version */
    #if ((CAN_FLEXCAN_AR_RELEASE_MAJOR_VERSION_C != SCHM_CAN_AR_RELEASE_MAJOR_VERSION) || \
        (CAN_FLEXCAN_AR_RELEASE_MINOR_VERSION_C != SCHM_CAN_AR_RELEASE_MINOR_VERSION))
        #error "AutoSar Version Numbers of FlexCan.c and SchM_Can.h are different"
    #endif
#endif
/*==================================================================================================
...
/* need to enclose this function for specific usage case: no HTHs configured */
#if ((CAN_TX_RX_INTR_SUPPORTED == STD_ON) || (CAN_TXPOLL_SUPPORTED == STD_ON))
...
#if (CAN_DEV_ERROR_DETECT == STD_ON)
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_ValidHth 
                                                            (
...
static FUNC (void, CAN_CODE) Can_FlexCan_InitBaudrate
(
    CONST(uint8, AUTOMATIC) u8HwOffset,
    CONST(uint8, AUTOMATIC) Controller,
    CONST(uint8, AUTOMATIC) u8BaudrateIndex
);
#if (CAN_FD_MODE_ENABLE == STD_ON)                                                              
...
LOCAL_INLINE FUNC(void, CAN_CODE) Can_FlexCan_DisableInterrupts (VAR(uint8, AUTOMATIC) u8Controller)
{
    ...
    if ((((uint32)1U << u8HwOffset) & CAN_FD_CONTROLLER_SUPPORTED) != 0U)
    {
        ...
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        REG_BIT_CLEAR32( FLEXCAN_CTRL2(u8HwOffset), FLEXCAN_FD_CTRL2_ERRMSK_FAST_U32);
    }
#endif
    ...
    */
    ...
    if ( CAN_CONTROLLERCONFIG_ERR_EN_U32 == (CAN_CONTROLLERCONFIG_ERR_EN_U32 & pCanControlerDescriptor->u32Options))
    {
        ...
        if ((((uint32)1U << u8HwOffset) & CAN_FD_CONTROLLER_SUPPORTED) != 0U)
        {
            ...
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
            REG_BIT_SET32( FLEXCAN_CTRL2(u8HwOffset), FLEXCAN_FD_CTRL2_ERRMSK_FAST_U32);
        }
#endif
    }
                                                            ...
                                                            )
{
    ...
    /* Calculate the index of the IFLAG register and bTxGuard (index=0 for MB32...MB63, index=1 for MB0...MB31, index=2 for MBO64 ... 96). */
                                                        ...
                                                        CONST(uint8, AUTOMATIC) u8IflagRegIndex)
{
    ...
    /* The interrupt flag is set, message was either transmitted or cancelled.
        This code does not check whether the bTxGuard is set.
                                                          ...
                                                          CONST(uint8, AUTOMATIC) u8IflagRegIndex )
{
    ...
    /*Array to store data in word*/
    VAR(uint32, AUTOMATIC)  u32CanMbData[16] = {0U};
    ...
    /* Variable for storing the ConfigReg of the MB. */
    VAR(uint32, AUTOMATIC)     u32MbConfig = 0U;
    VAR(uint8, AUTOMATIC) u8NumOfWord = 0U;
    VAR(uint8, AUTOMATIC) u8WordIdx = 0U;
    VAR(uint32, AUTOMATIC)          u32MbAddress = 0U;
    /* Pointer to the MB container structure. */
    VAR(Can_PtrMBConfigContainerType, AUTOMATIC)   pCanMbConfigContainer = NULL_PTR;
    ...
    u8HwOffset = CanStatic_pControlerDescriptors[controller].u8ControllerOffset;
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
    u32MbAddress = FLEXCAN_MB(u8HwOffset, u16MBGlobalIndex) + (uint32)8U;
...
#ifdef ERR_IPV_FLEXCAN_0017
    #if (ERR_IPV_FLEXCAN_0017 == STD_ON)
        #if (CAN_RXFIFO_ENABLE == STD_ON)
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            if (CAN_CONTROLLERCONFIG_RXFIFO_U32 == ((Can_pControlerDescriptors[controller].u32Options) & CAN_CONTROLLERCONFIG_RXFIFO_U32)) 
            {
                /* The errata just impact when legacy fifo is enable */
                /* Disable all interrupts to have control over the Rx Message Buffer (MBx) locking time. */
    ...
    u8NumOfWord = u8MbDataLength >> 2U;
    /* remainder by 4 */
    if ((uint8)0U != (u8MbDataLength & FLEXCAN_MASK_2BITS_U8))
    {
        u8NumOfWord++;
    }
    /*
    ...
    */
    for (u8WordIdx = 0U; u8WordIdx < u8NumOfWord; u8WordIdx++)
    { 
        /* Copy the data */
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u32CanMbData[u8WordIdx] = REG_AREAD32(u32MbAddress, (uint32)u8WordIdx);
    }
...
if(FLEXCAN_MBCS_CODERXOVRR_U32 == (u32MbConfig & FLEXCAN_MBCS_CODE_U32))
{
    ...
    for (u8WordIdx = 0U; u8WordIdx < u8NumOfWord; u8WordIdx++)
    { 
        u8CanMbData[u8DatabyteIndex] = (uint8)((u32CanMbData[u8WordIdx] >> 24U) & ((uint32)0xFF));
        u8CanMbData[u8DatabyteIndex + (uint8)1U] = (uint8)((u32CanMbData[u8WordIdx] >> 16U) & ((uint32)0xFF));
        u8CanMbData[u8DatabyteIndex + (uint8)2U] = (uint8)((u32CanMbData[u8WordIdx] >> 8U) & ((uint32)0xFF));
        u8CanMbData[u8DatabyteIndex + (uint8)3U] = (uint8)((u32CanMbData[u8WordIdx]) & ((uint32)0xFF));
        u8DatabyteIndex += 4U;
    }
                                                        ...
                                                        CONST(uint32, AUTOMATIC)  u32TempFlag)
{
    ...
    VAR(uint8, AUTOMATIC) u8WordIdx = 0U;
    VAR(uint32, AUTOMATIC)          u32MbAddress = 0U;
    VAR(uint32, AUTOMATIC)          u32RegVal = 0U;
    /* Variable for indexing all the MBs fron hardware unit chain (all controllers). */
    VAR(uint16, AUTOMATIC)            u16MBGlobalIndex = 0U;
    VAR(Can_PtrMBConfigContainerType, AUTOMATIC)   pCanMbConfigContainer = NULL_PTR;
    /* Identifies the HRH and its corresponding CAN Con-troller. */
    ...
    if ( FLEXCAN_FIFOFRAME_INT_INDEX_U8 == (uint32)u8MbIndex )
    {
        ...
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u16MBGlobalIndex = Can_pControlerDescriptors[controller].Can_u16RxFiFoHrhFirst;
        do
        {
            ...
            */
            ...
            for (u8WordIdx = 0U; u8WordIdx < u8NumOfWord; u8WordIdx++)
            {
                ...
                /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                u32RegVal = REG_AREAD32(u32MbAddress, (uint32)u8WordIdx);
                u8CanMbData[u8DatabyteIndex] = (uint8)((u32RegVal >> 24U) & ((uint32)0xFF));
                u8CanMbData[u8DatabyteIndex + (uint8)1U] = (uint8)((u32RegVal >> 16U) & ((uint32)0xFF));
                u8CanMbData[u8DatabyteIndex + (uint8)2U] = (uint8)((u32RegVal >> 8U) & ((uint32)0xFF));
                u8CanMbData[u8DatabyteIndex + (uint8)3U] = (uint8)((u32RegVal) & ((uint32)0xFF));
                u8DatabyteIndex += 4U;
            }
            /* Determine Hrh of the message. */
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                                                            ...
                                                            )
{
    ...
    (void)bEnabledFD;   /* Prevent compiler warning when CAN_FD is disabled */
    (void)u16MBGlobalIndex; /* Prevent compiler warning when CAN_FD is disabled */
    return eReturnValue;
}
#endif /* (CAN_DEV_ERROR_DETECT == STD_ON) */
...
static FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_SellectClockSource( VAR(uint8, AUTOMATIC) u8Controller, VAR(uint8, AUTOMATIC) u8HwOffset, VAR(uint8, AUTOMATIC) u8BaudrateIndex )
{
    ...
    {
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u32ValueControlRegister = CanStatic_pControlerDescriptors[u8Controller].pStaticControllerBaudrateConfigsPtr[u8BaudrateIndex].u32ControlRegister;

        SchM_Enter_Can_CAN_EXCLUSIVE_AREA_09();
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        REG_WRITE32( FLEXCAN_CTRL(u8HwOffset), (uint32)(((uint32)(u32ValueControlRegister & FLEXCAN_CTRL_CLKSRC_MASK_U32)) & FLEXCAN_CTRL_CONFIG_MASK_U32));
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        REG_BIT_CLEAR32( FLEXCAN_MCR(u8HwOffset), FLEXCAN_MCR_MDIS_U32); 
        SchM_Exit_Can_CAN_EXCLUSIVE_AREA_09();
...
)
{
    /* Local copy of pointer to the controller descriptor. */
    VAR(Can_PtrControlerDescriptorType, AUTOMATIC) pCanControlerDescriptor = NULL_PTR;
    VAR(uint16, AUTOMATIC) u16Prescaler = 0U;
    VAR(uint32, AUTOMATIC)           u32ValueControlRegister = 0U;
    ...
    {
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u16Prescaler = pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].u16CtrlPrescalerAlt;
    }
    else
    {
#endif /* (CAN_DUAL_CLOCK_MODE == STD_ON) */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u16Prescaler = pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].u16CtrlPrescaler;
#if (CAN_DUAL_CLOCK_MODE == STD_ON)
    }
#endif /* (CAN_DUAL_CLOCK_MODE == STD_ON) */
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
    u32ValueControlRegister = CanStatic_pControlerDescriptors[Controller].pStaticControllerBaudrateConfigsPtr[u8BaudrateIndex].u32ControlRegister;
    SchM_Enter_Can_CAN_EXCLUSIVE_AREA_05();
    /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
    /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
    /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
    REG_WRITE32( FLEXCAN_CTRL(u8HwOffset), (u32ValueControlRegister & FLEXCAN_CTRL_CONFIG_MASK_U32));
    SchM_Exit_Can_CAN_EXCLUSIVE_AREA_05();
    ...
    if ((uint32)(FLEXCAN_MCR_FDEN_U32) == (pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].ControllerFD.u32CanFdEnable))
    {
        /* Init CBT register */
       /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u32ValueControlRegister = pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].u32ControlRegister;
        u32ValueControlRegister |= ((uint32)u16Prescaler << FLEXCAN_FD_PRESDIV_CBT_OFFSET_U8);
        
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        REG_WRITE32( FLEXCAN_CBT(u8HwOffset), u32ValueControlRegister);
        ...
        if (FLEXCAN_FDCTRL_FDRATE_U32 == (pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].ControllerFD.u32CanControllerTxBitRateSwitch))
        {
            ...
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            u32ValueControlRegister = pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].ControllerFD.u32CanFdCbtRegister;
            u32ValueControlRegister |= ((uint32)u16Prescaler << FLEXCAN_FD_PRESDIV_OFFSET_U8);
            /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            REG_WRITE32(FLEXCAN_FDCBT(u8HwOffset), u32ValueControlRegister);
        #if (CAN_DUAL_CLOCK_MODE == STD_ON)
            if (Can_ControllerStatuses[Controller].CanClockMode == CAN_ALTERNATE)
            {
                /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                u32ValueControlRegister = (pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].ControllerFD.u32CanControllerTrcvDelayCompensationAlt) << FLEXCAN_FDCTRL_TDCOFF_OFFSET_U8;
            }
            else
            {
        #endif /* (CAN_DUAL_CLOCK_MODE == STD_ON) */
                /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                u32ValueControlRegister = (pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].ControllerFD.u32CanControllerTrcvDelayCompensation) << FLEXCAN_FDCTRL_TDCOFF_OFFSET_U8;
       #if (CAN_DUAL_CLOCK_MODE == STD_ON)
            }
        #endif /* (CAN_DUAL_CLOCK_MODE == STD_ON) */
            if ((uint32)0U != u32ValueControlRegister)
            {
                ...
                /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
                /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                REG_RMW32(FLEXCAN_FDCTRL(u8HwOffset), FLEXCAN_FDCTRL_TDCOFF_U32, u32ValueControlRegister);
            }
            else
            {
                /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
    ...
    else
    {
        ...
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        u32ValueControlRegister = pCanControlerDescriptor->pControllerBaudrateConfigsPtr[u8BaudrateIndex].u32ControlRegister;
        u32ValueControlRegister |= ((uint32)u16Prescaler << FLEXCAN_CTRL_PRESDIV_SHIFT_U8);
        SchM_Enter_Can_CAN_EXCLUSIVE_AREA_05();
        /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
        /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
        /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        REG_RMW32( FLEXCAN_CTRL(u8HwOffset), FLEXCAN_CTRL_BIT_TIME_FIELD_U32, u32ValueControlRegister);
        SchM_Exit_Can_CAN_EXCLUSIVE_AREA_05();
#if (CAN_FD_MODE_ENABLE == STD_ON)
    }
#endif
...
FUNC (Std_ReturnType, CAN_CODE) Can_FlexCan_InitController( VAR(uint8, AUTOMATIC) Controller)
{
    ...
    if ((Std_ReturnType)E_OK == eResetReturn)
    {
        ...
        Can_FlexCan_InitBaudrate(u8HwOffset, Controller, u8BaudrateIndex);
...
*/            
        ...
        else /* RxFIFO is disabled */
        {
            ...
            Can_ControllerStatuses[Controller].u32CancelMBIndex = (Can_HwHandleType)CAN_MAXMB_SUPPORTED;
        }
    #else
        #error : the define ERR_IPV_FLEXCAN_0015 must not be STD_OFF
    #endif
        ...
        do
        {
            Can_ControllerStatuses[Controller].bTxGuard[u8RegCount] = (boolean)FALSE;
            u8RegCount++;
        }
        while(u8RegCount < (uint8)CAN_MAXMB_SUPPORTED);
    ...
    /* Do not set the Abort Enable bit of the MCR */
        ...
        if ((((uint32)1U << u8HwOffset) & CAN_FD_CONTROLLER_SUPPORTED) != 0U)
        {
            /* clear flag FAST error interrupt */
            /* @violates @ref Can_Flexcan_c_REF_9 Violates MISRA 2004 Advisory Rule 11.3, A cast should not be performed */
            /* @violates @ref Can_Flexcan_c_REF_2 Violates MISRA 2004 Required Rule 11.1, cast from unsigned long to pointer. */
            /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
            REG_WRITE32( FLEXCAN_ESR(u8HwOffset), (FLEXCAN_ESR_ERRINT_FAST_U32 & FLEXCAN_ESR_CONFIG_MASK_U32));
        }
#endif
...
FUNC(void, CAN_CODE) Can_FlexCan_AbortMb( VAR(Can_HwHandleType, AUTOMATIC) Hth )
{
    ...
    /* Controller ID must be within range and the selected Controller must be configured for use in the config tool. */
     /* @violates @ref Can_Flexcan_c_REF_6 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
    if (CAN_NULL_OFFSET == (CanStatic_pControlerDescriptors[u8Controller].u8ControllerOffset))
    {
...
#if (CAN_FD_MODE_ENABLE == STD_ON)
    eReturnValue = Can_FlexCan_ValidHth( PduInfo, u16MBGlobalIndex, bEnabledFD);
#else
    eReturnValue = Can_FlexCan_ValidHth( PduInfo, u16MBGlobalIndex, (boolean)FALSE);
#endif /* (CAN_FD_MODE_ENABLE == STD_ON)   */
    if ((Std_ReturnType)E_OK == eReturnValue)
    {
        ...
        do
        {
            ...
            /* Is the MB empty?
                 This test is a bit early; the bTxGuard is not set yet and someone else can grab this MB before we reserve it
            ...
            if ( FLEXCAN_MBCS_CODETX_U32 == (uint32)(REG_READ32( FLEXCAN_MB((u8HwOffset), (uint32)(u16MBGlobalIndex))) & (uint32)(FLEXCAN_MBCS_CODE_U32)) )
            {
                ...
                */
                ...
                if ( (boolean)FALSE == (Can_ControllerStatuses[u8CtrlId].bTxGuard[u8MbIndex]) )
                {
                    ...
                    Can_ControllerStatuses[u8CtrlId].bTxGuard[u8MbIndex] = (boolean)TRUE;
                    ...
                    Can_ControllerStatuses[u8CtrlId].bTxGuard[u8MbIndex] = (boolean)FALSE;
                }
                else
                {

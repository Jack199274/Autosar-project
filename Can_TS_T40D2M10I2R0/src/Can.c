...
#define CAN_SW_PATCH_VERSION_C               2
                                        ...
                                        )
{
    ...
    else
    {
        ...
        /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
        if ((Hth < Can_pCurrentConfig->uCanFirstHTHIndex) || (Hth >= (Can_HwHandleType)CAN_MAX_OBJECT_ID))
        {
            /*
...
*/
/* @violates @ref Can_c_REF_5 Violates MISRA 2004 Required Rule 8.10, external ... could be made static */
FUNC(void, CAN_CODE) Can_SetClockMode(VAR(Can_ClockModeType, AUTOMATIC) can_clk_mode)
{
    /* Variable for indexing the controllers */
    VAR(uint8, AUTOMATIC) u8controllerId = 0U;
    VAR(boolean, AUTOMATIC) bCtrlBusy = (boolean)FALSE;
    ...
    else
    {
        ...
        {
            (void)Det_ReportError( (uint16)CAN_MODULE_ID, (uint8)CAN_INSTANCE, (uint8)CAN_SID_SETCLOCKMODE, (uint8)CAN_E_PARAM_HANDLE);
        }
        else
        {
#endif
            /* Loop through all Can controllers configured based CanControllerId parameter. */
            for ( u8controllerId = 0U; u8controllerId < (uint8)CanStatic_pCurrentConfig->u8ControllersConfigured; u8controllerId++)
            {
                /* @violates @ref Can_c_REF_4 Violates MISRA 2004 Required Rule 17.4, pointer arithmetic other than array indexing used */
                if (CAN_NULL_OFFSET != (CanStatic_pControlerDescriptors[u8controllerId].u8ControllerOffset))
                {
                    if ( CAN_CS_STARTED == Can_ControllerStatuses[u8controllerId].ControllerState )
                    {
                    #if (CAN_DEV_ERROR_DETECT == STD_ON)
                        (void)Det_ReportError((uint16)CAN_MODULE_ID, CAN_INSTANCE, (uint8)CAN_SID_SETCLOCKMODE , CAN_E_TRANSITION);
                    #endif
                        bCtrlBusy = (boolean)TRUE;
                        break;
                    }
                }
            }
            if ((boolean)FALSE == bCtrlBusy)
            {
                /* Loop through all Can controllers configured based CanControllerId parameter. */
                for ( u8controllerId = 0U; u8controllerId < (uint8)CanStatic_pCurrentConfig->u8ControllersConfigured; u8controllerId++)
                {
                    ...
                    if (CAN_NULL_OFFSET != (CanStatic_pControlerDescriptors[u8controllerId].u8ControllerOffset))
                    {
                        Can_ControllerStatuses[u8controllerId].CanClockMode = can_clk_mode;
                    }
                }
            }
#if (CAN_DEV_ERROR_DETECT == STD_ON)

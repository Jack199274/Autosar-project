...
#define SPI_SLAVE_SUPPORT [!//
[!IF "(SpiGeneral/SpiLevelDelivered != 0)"!][!//
 ([!IF "$SPI_SLAVE_FLAG = 1"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!])
[!ELSE!][!//
 (STD_OFF)
[!ENDIF!][!//
...
*/
#define SPI_ENABLE_FLEXIO ([!IF "(count(SpiGeneral/SpiPhyUnit/*/SpiPhyUnitMapping[.='FLEXIOSPI_0']) + count(SpiGeneral/SpiPhyUnit/*/SpiPhyUnitMapping[.='FLEXIOSPI_1'])) != 0 "!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
/**
* @brief Defines if Spi driver optimization for sequences having only one job is activated or not.
* @details Defines if Spi driver optimization for sequences having only one job is activated or not.
*        If activated, additional RAM memory is required for internal data caching.
...
*/
#define SPI_ALLOW_BIGSIZE_COLLECTIONS ([!IF "(node:value(SpiAutosarExt/SpiAllowBigSizeCollections) = 'true')"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

...
#define SPI_SW_PATCH_VERSION_C              2
...
#error "Software Version Numbers of Spi.c and Spi_IPW.h are different"
    ...
    #error "AutoSar Version Numbers of Spi.c and Det.h are different"
     ...
     /* Check if current file and Dem.h header file are of the same Autosar version */
        ...
        #endif
    #endif
    /* Check if current file and SchM_Spi.h header file are of the same Autosar version */
...
#define SPI_JOB_NULL            ((Spi_JobType)(0xFFFFU))
    #endif /* (SPI_ALLOW_BIGSIZE_COLLECTIONS == STD_ON) */
#endif /* ((SPI_LEVEL_DELIVERED == LEVEL1) || (SPI_LEVEL_DELIVERED == LEVEL2)) */
 ...
 */
static volatile VAR(uint32, SPI_VAR) Spi_aSpiBusySyncHWUnitsStatus[SPI_MAX_HWUNIT] = {0u};
    ...
    )
{
...
#if (SPI_DEV_ERROR_DETECT == STD_ON)
    /* Check whether the Spi driver is in UNINIT state */
    if (NULL_PTR != Spi_pcSpiConfigPtr)
    {
        /* Spi driver has been already initialized */
        /*
        * @violates @ref Spi_c_REF_9 If a function returns error information,
        *           then that error information shall be tested.
        */
 ...
 #if (SPI_DEV_ERROR_DETECT == STD_ON)
    /* Report error if ConfigPtr is passed as a NULL Pointer */
        if (NULL_PTR == ConfigPtr)
        {
            ...
            *           then that error information shall be tested.
            */
            Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_INIT_ID,SPI_E_PARAM_POINTER);
        }
        else
        {
#endif /* (SPI_DEV_ERROR_DETECT == STD_ON) */
        ...
        else
        {
             ...
             (void)ConfigPtr; /* Avoid compiler warning */
...
FUNC(Spi_StatusType, SPI_CODE) Spi_GetStatus(void)
{
    ...
    else
    {
        ...
        for (HWUnit = 0u; HWUnit < (Spi_HWUnitType)SPI_MAX_HWUNIT; HWUnit++)
        {
            if (1u == Spi_aSpiBusySyncHWUnitsStatus[HWUnit])
            {
                StatusFlag = SPI_BUSY;
                break;
            }
        ...
        if ((Std_ReturnType)E_NOT_OK != Status)
        {
            ...
            */
            ...
            for (HWUnit = 0u; HWUnit < (Spi_HWUnitType)SPI_MAX_HWUNIT; HWUnit++)
            {
                ...
                * @violates @ref Spi_c_REF_6 Array indexing shall be the only allowed form of pointer arithmetic.
                */
                if(0u != ((Spi_aSpiBusySyncHWUnitsStatus[HWUnit] << Spi_pcSpiConfigPtr->pcHWUnitConfig[HWUnit].u8Offset) & u32SpiSequenceUsedHWUnits))
                {
                    Status = (Std_ReturnType)E_NOT_OK;
                }
                else
            ...
            if ((Std_ReturnType)E_NOT_OK != Status)
            {
                ...
                for (JobIndex = 0u; JobIndex < NumJobsInSequence; JobIndex++)
                {
                    ...
                    */
                    ...
                    /* Logical Spi HWUnit */
                    HWUnit = pcJobConfig->HWUnit;
                    Spi_aSpiBusySyncHWUnitsStatus[HWUnit] = 1u;
                }
                ...
                for (JobIndex = 0u; JobIndex < NumJobsInSequence; JobIndex++)
                {
                    ...
                    */
                    ...
                    /* Logical Spi HWUnit */
                    HWUnit = pcJobConfig->HWUnit;
                    Spi_aSpiBusySyncHWUnitsStatus[HWUnit] = 0u;
                }
    ...
    )
{
    ...
    if(SPI_JOB_OK != pJobState->Result)
    {
        /* set job result OK */
        pJobState->Result = SPI_JOB_OK;
        SchM_Exit_Spi_SPI_EXCLUSIVE_AREA_07();

        pSequenceState = pJobState->pAsyncCrtSequenceState;
        pcSequenceConfig = pSequenceState->pcSequence;

...
#ifdef SPI_SWAP32BIT_ENABLE
    /**< @brief Implementation specific field referencing the channel internal state. */
    VAR(uint8,SPI_VAR) Swap32BitEnable;
#endif
#ifdef SPI_SLAVE_HIGH_SPEED_SUPPORT
#if(SPI_SLAVE_HIGH_SPEED_SUPPORT == STD_ON)
#if ((SPI_DMA_USED == STD_ON) && \
...
#endif
#endif
#ifdef SPI_MTFE_MODE_SUPPORTED
#if (STD_ON == SPI_MTFE_MODE_SUPPORTED)
    VAR(uint8, SPI_VAR) u8ModTranferEnable;
#endif
#endif
#ifdef SPI_FLEXIO_USED
...
typedef struct
{
    ...
    P2CONST(Spi_Ipw_AttributesConfigType, SPI_VAR, SPI_APPL_CONST) pcAttributesConfig;
    ...
    CONST(Mcal_DemErrorType, SPI_CONST) Spi_E_Timeout_ErrorCfg;
#endif

...
#define LIN_FLEXIO_SW_PATCH_VERSION_C               2
/*==================================================================================================
*                                      FILE VERSION CHECKS
==================================================================================================*/
...
/* Check if current file and Lin_FlexIO.h header file are of the same Software version */
    ...
    /* Check if current file and CDD_Mcl.h header file are of the same Autosar version */
        ...
        #if ((LIN_FLEXIO_AR_RELEASE_MAJOR_VERSION_C  != DEM_AR_RELEASE_MAJOR_VERSION) || \
             (LIN_FLEXIO_AR_RELEASE_MINOR_VERSION_C  != DEM_AR_RELEASE_MINOR_VERSION) \
            )
            #error "AutoSar Version Numbers of Lin_FlexIO.c and Dem.h are different"
        #endif
    ...
    #if ((LIN_FLEXIO_AR_RELEASE_MAJOR_VERSION_C != SCHM_LIN_AR_RELEASE_MAJOR_VERSION) || \
        (LIN_FLEXIO_AR_RELEASE_MINOR_VERSION_C != SCHM_LIN_AR_RELEASE_MINOR_VERSION))
        #error "AutoSar Version Numbers of Lin_FlexIO.c and SchM_Lin.h are different"
    #endif
...
static FUNC (Lin_StatusType, LIN_CODE) Lin_FlexIO_DataTransmissionGetStatus(CONST(uint8, AUTOMATIC) u8Channel);
static FUNC (void, LIN_CODE) Lin_FlexIO_SlaveResponseInterruptHandler(CONST(uint8, AUTOMATIC) u8LogicalChannel, \
...
/**
 * @brief   Gets the status of the LIN driver when data is transmitting.
...
/**
* @brief   Interrupt handler for RX Data Register Full on FlexIO in slave response.
* @details This function shall manage the RX ISRs on the
*          addressed channel if the frame command is a slave response command.
*
* @param[in]    Channel LIN channel to be addressed.
                                                                     ...
                                                                     )
{
    ...
    VAR(uint32, AUTOMATIC) u32WriteValue;
...
{
    VAR(uint8, AUTOMATIC) u8TimeStatusReg;
    ...
    /* Check whether wake up detection is enabled */
     if ( (uint8) STD_ON == Lin_pChannelConfigPtr[u8Channel]->pChannelConfigPC->u8LinChannelWakeupSupport)
     {
        ...
        /* Check Wakeup event */
        u8TimeStatusReg = Mcl_Flexio_ReadTimStat((uint8)(1U << u32WriteValue));

        if (0U != u8TimeStatusReg)
        {
...
FUNC(Lin_StatusType, LIN_CODE) Lin_FlexIO_HardwareGetStatus(CONST(uint8, AUTOMATIC) u8Channel, P2VAR(uint8, AUTOMATIC, LIN_APPL_DATA) pu8LinSduPtr)
{
    ...
    if(((uint8)0U) != u8Temp2)
    {
        ...
        {
            u8TempReturn = Lin_FlexIO_DataTransmissionGetStatus(u8Channel);
        }
                                                ...
                                                )
{
    ...
    VAR(boolean, AUTOMATIC) bIsSpuriousIrq = (boolean)FALSE;
    ...
    /* Check whether spurious occurred or not */
    if ((boolean)TRUE == bIsSpuriousIrq)
    {
        if(FLEXIO_0_CH_0 == u8FlexIO_Channel)
        {
            Mcl_Flexio_ClearShiftStat((uint8)FLEXIO_SEL_0_1_MASK_U32);
        }
        else if(FLEXIO_0_CH_2 == u8FlexIO_Channel)
        {
...
static FUNC (void, LIN_CODE) Lin_FlexIO_RxInterruptHandler(VAR(uint8, AUTOMATIC) u8LogicalChannel)
{
    ...
    */
    ...
    else
    {
            ...
            default:
                switch(au8Lin_Command)
                {
                    ...
                    case LIN_TX_SLAVE_RES_COMMAND:
                        Lin_FlexIO_SlaveResponseInterruptHandler(u8LogicalChannel, u32Lin_DataReg);
                        break;
        ...
        /* Check whether any errors occurred when sleep command had been sent */

...
#define LIN_LPUART_SW_PATCH_VERSION_C               2
/*==================================================================================================
*                                      FILE VERSION CHECKS
==================================================================================================*/
...
/* Check if current file and Reg_eSys_LPUART.h header file are of the same Software version */
  ...
  #if ((LIN_LPUART_AR_RELEASE_MAJOR_VERSION_C != SCHM_LIN_AR_RELEASE_MAJOR_VERSION) || \
      (LIN_LPUART_AR_RELEASE_MINOR_VERSION_C != SCHM_LIN_AR_RELEASE_MINOR_VERSION))
...
static FUNC (Lin_StatusType, LIN_CODE) Lin_LPUART_DataTransmissionGetStatus(CONST(uint8, AUTOMATIC) u8Channel, CONST(uint8, AUTOMATIC) u8ChannelFrameError);
static FUNC (Lin_StatusType, LIN_CODE) Lin_LPUART_HeaderTransmissionGetStatus(CONST(uint8, AUTOMATIC) u8Channel, CONST(uint8, AUTOMATIC) u8ChannelFrameError);
static FUNC (void, LIN_CODE) Lin_LPUART_SlaveResponseInterruptHandler(CONST(uint8, AUTOMATIC) u8LPUART_Channel, \
                                                                      CONST(uint8, AUTOMATIC) u8LogicalChannel, \
                                                                      CONST(uint32, AUTOMATIC) u32Lin_DataReg \
                                                                     );
/*==================================================================================================
*                                       LOCAL FUNCTIONS
==================================================================================================*/
...
static FUNC (void, LIN_CODE) Lin_LPUART_RxBuffFullInterruptHandler(CONST(uint8, AUTOMATIC) u8LPUART_Channel, VAR(uint8, AUTOMATIC) u8LogicChannel)
{
    ...
    */
    ...
    else
    {
            ...
            default:
                switch(au8Lin_Command)
                {
                    ...
                    case LIN_TX_SLAVE_RES_COMMAND:
                        /* Handle interrupt if the frame is a slave response frame */
                        Lin_LPUART_SlaveResponseInterruptHandler(u8LPUART_Channel, u8LogicalChannel, u32Lin_DataReg);
                        break;
        ...
        if ((LIN_NO_ERROR !=Lin_au8LinChFrameErrorStatus[u8LogicalChannel]) && \
            (LIN_TX_SLEEP_COMMAND == au8Lin_Command) \
           )
        {
    ...
    if (LIN_CH_SLEEP_STATE == Lin_au8LinChStatus[u8LogicalChannel])
    {
        ...
        */
         ...
         if ((uint8)STD_ON == Lin_pChannelConfigPtr[u8LogicalChannel]->pChannelConfigPC->u8LinChannelWakeupSupport)
        {
            ...
            Lin_au8LinChFrameErrorStatus[u8LogicalChannel] = LIN_NO_ERROR;
        }
    }
    else
    {
...
/**
* @brief   Interrupt handler for RX Data Register Full on LPUART in slave responses command.
* @details This function shall manage the RX ISRs on the
*          addressed channel when the frame command is a slave response command.
*
* @param[in]   LIN hardware channel to be addressed.
* @param[in]   LIN logical channel to be addressed.
* @param[in]   Value of the data register.
*
* @return       void.
*
* @note         Internal driver function.
 ...
 *
 * @param[in]   Channel LIN channel to be addressed
 * @param[in]   Frame transmission error status of the Lin Channel
 *
...
static FUNC (Lin_StatusType, LIN_CODE) Lin_LPUART_DataTransmissionGetStatus(CONST(uint8, AUTOMATIC) u8Channel, CONST(uint8, AUTOMATIC) u8ChannelFrameError)
{
    ...
    switch(u8ChannelFrameError)
    {
        ...
        case LIN_NO_ERROR:
            ...
            {
                if (LIN_RX_ONGOING_STATE == Lin_au8LinChFrameStatus[u8Channel])
                {
 ...
 *
 * @param[in]   Channel LIN channel to be addressed
 * @param[in]   Header transmission error status of the Lin Channel
 *
 * @return                      Lin_StatusType.
 * @retval LIN_NOT_OK           Development or production error raised
 *                              none of the below conditions.
...
FUNC(void, LIN_CODE) Lin_LPUART_InitChannel(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    */
    ...
    SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_00();
    {
        ...
        * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
        */
        REG_RMW32(LPUART_CTRL_ADDR32(u8LPUART_Channel), LPUART_CTRL_IE_FLAGS_MASK_U32, (uint32)0x0);
    ...
    if((uint8)E_OK == u8TmpReturn)
    {
        ...
        else
        {
            ...
            SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_02();
            {
                ...
                * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
                */
                REG_WRITE32(LPUART_DATA_ADDR32(u8LPUART_Channel), LPUART_DATA_BRK_VALUE_U32);
    ...
    if((uint8)E_OK == u8TmpReturn)
    {
        ...
        SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_05();
        {
            ...
            */
            ...
            /* Transimit break character as speacial character */
            /**
            *
            * @violates @ref Lin_LPUART_c_REF_3 cast from unsigned int to pointer
            *
...
FUNC (void, LIN_CODE) Lin_LPUART_WakeUp(CONST(uint8, AUTOMATIC) u8Channel)
{
    ...
    SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_08();
    {
        ...
        */
        ...
        if (LIN_CH_SLEEP_STATE == Lin_au8LinChStatus[u8Channel])
        {
            Lin_Lpuart_au8BufferLength[u8Channel] = (uint8)0U;
        }
...
FUNC(Lin_StatusType, LIN_CODE) Lin_LPUART_HardwareGetStatus(CONST(uint8, AUTOMATIC) u8Channel, P2VAR(uint8, AUTOMATIC, LIN_APPL_DATA) pu8LinSduPtr)
{
    ...
    if(((uint8)0U) != u8Temp2)
    {
        ...
        {
            u8TempReturn = Lin_LPUART_HeaderTransmissionGetStatus(u8Channel, u8ChannelFrameError);
        }
        /* Data transmission */
        else if(u8Temp2 > u8Temp1)
        {
            u8TempReturn = Lin_LPUART_DataTransmissionGetStatus(u8Channel, u8ChannelFrameError);
        }
                       ...
                       */
                        ...
                        */
                        ...
                        if (LIN_CH_SLEEP_STATE == u8Temp1)
                        {
                            ...
                            * @violates @ref Lin_LPUART_c_REF_7 cast should not be performed
                            */
                            REG_RMW32(LPUART_CTRL_ADDR32(u8LPUART_Channel), LPUART_CTRL_IDLECFG_MASK_U32, (uint32)0x0);

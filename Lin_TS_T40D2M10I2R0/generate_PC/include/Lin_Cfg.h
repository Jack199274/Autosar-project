...
* external identifiers.
*/
#define LIN_AR_RELEASE_MINOR_VERSION_CFG     3
/**
* @violates @ref Lin_Cfg_h_REF_2 The compiler/linker shall be checked to
* ensure that 31 character signifiance and case sensitivity are supported for
* external identifiers.
*/
#define LIN_AR_RELEASE_REVISION_VERSION_CFG  1
...
* @api
*/
#define LIN_MAX_DATA_LENGTH  [!"AutosarExt/LinMaxDataLength"!]U
...
* @details        When AutosarExt/LinFrameTimeoutDisable == STD_ON,
*                 the LIN Master will accept the frame that is longer than TFrame_Maximum.
*
* @api
*/
#define LIN_FRAME_TIMEOUT_DISABLE   [!IF "AutosarExt/LinFrameTimeoutDisable"!](STD_ON)  /* The LIN Master will accept the frame that is longer than TFrame_Maximum. */[!ELSE!](STD_OFF) /* The master will not accept the frame that is longer than TFrame_Maximum. */[!ENDIF!]

[!IF "(node:exists(AutosarExt/LinStartupState))"!]
/**
* @brief          The start up state.
* @details        When AutosarExt/LinStartupState == SLEEP, LIN_START_UP_SLEEP_STATE is set == STD_ON and the LIN driver will be set LIN_CH_SLEEP mode after inition.
*                 Or when AutosarExt/LinStartupState == NORMAL, LIN_START_UP_SLEEP_STATE is set == STD_OFF and the LIN driver will be set LIN_CH_OPERATIONAL mode after inition.
*/
#define LIN_START_UP_SLEEP_STATE [!IF "(AutosarExt/LinStartupState = 'SLEEP')"!] (STD_ON) /* The Lin driver will be set to LIN_CH_SLEEP after Lin_init function called */ [!ELSE!] (STD_OFF) /* The Lin driver will be set to LIN_CH_OPERATIONAL after Lin_init function called */ [!ENDIF!]
[!ENDIF!][!//
...
[!IF "var:defined('postBuildVariant')"!][!//
    ...
    [!//
        [!CODE!][!WS4!]extern CONST(Lin_ConfigType, LIN_CONST)  Lin_Config_[!"."!];[!IF "$variantIndex < $variantNumber"!]\[!ENDIF!][!CR!][!ENDCODE!]
    [!ENDLOOP!][!//
[!ELSE!][!//
    [!CODE!][!WS4!]extern CONST(Lin_ConfigType, LIN_CONST)  Lin_Config;[!CR!][!ENDCODE!]
[!ENDIF!][!//
[!ENDNOCODE!][!//
[!ENDMACRO!]
...
[!NOCODE!][!//
[!VAR "LinAllChEcuMWakeupSourceDisable"="'false'"!]
[!LOOP "LinGlobalConfig/LinChannel/*"!]
    [!IF "not(node:exists(LinChannelEcuMWakeupSource))"!]
        [!VAR "LinAllChEcuMWakeupSourceDisable"="'true'"!]
    [!ENDIF!]
[!ENDLOOP!]
[!IF "$LinAllChEcuMWakeupSourceDisable"!]
    [!CODE!]
/**

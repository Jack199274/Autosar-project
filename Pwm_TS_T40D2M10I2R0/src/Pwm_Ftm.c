...
#define PWM_FTM_SW_PATCH_VERSION_C              2
/**@}*/
...
===============================================================================================*/
    ...
    #if ((PWM_FTM_AR_RELEASE_MAJOR_VERSION_C != SCHM_PWM_AR_RELEASE_MAJOR_VERSION) || \
         (PWM_FTM_AR_RELEASE_MINOR_VERSION_C != SCHM_PWM_AR_RELEASE_MINOR_VERSION))
        #error "AutoSar Version Numbers of Pwm_Ftm.c and SchM_Pwm.h are different"
    #endif
#endif
...
#define PWM_FTM_SWOCTRL_BITS_OFFSET_U16             ((uint16)8U)
/*=================================================================================================
*                                      GLOBAL CONSTANTS
=================================================================================================*/
...
/**
* @brief Array of values for the modulo register value for each module
*/
static VAR(uint16, PWM_VAR) Pwm_Ftm_au16ModRegValue[PWM_FTM_MODULE_NO];
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief The phase-shift value of the Ftm module used for API
*/    
static VAR(uint16, PWM_VAR)   Pwm_Ftm_u16PhaseShift_TimeBase;
#endif
...
*/
static  VAR(Pwm_OutputStateType, PWM_VAR) Pwm_Ftm_eChannelPolarity[PWM_FTM_CHANNELS_MAX_U8];
#endif
...
* @brief Array holding the configured polarity for all Ftm channels
*/
static  VAR(Pwm_OutputStateType, PWM_VAR) Pwm_Ftm_eChannelIdleState[PWM_FTM_CHANNELS_MAX_U8];
#endif
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief The variable will be used to check interrupt when using SetPhaseShift
*/    
static VAR(uint8, PWM_VAR)   Pwm_Ftm_u8SetPhaseShiftCheck = 0xFF;
#endif
...
/*=============================================================================================*/
/**
* @brief        This function calculates the value of the CnV register depending on
*               the edge alignment mode (except phase-shifted mode).
*
* @param        nHwModuleIdx        FTM hardware module index
*               u16DutyCycle        Duty cycle value
*               eAlignment          Edge alignment type
*
* @return       Pwm_Ftm_ModuleType
*
*/
static FUNC(uint16, PWM_CODE) Pwm_FtmInternal_CalculateCnVReg
(
    VAR         (Pwm_Ftm_ModuleType,        AUTOMATIC)                      nHwModuleIdx,
...
/**
* @brief        This function calculates the value of the CnV register for channel (n+1)
*               in phase-shifted mode
*
* @param        nHwModuleIdx        FTM hardware module index
*               u16DutyCycle        Duty cycle value
*               u16PhaseShift       Phase shift value
*
* @return       Pwm_Ftm_ModuleType
*
*/
static FUNC(uint16, PWM_CODE) Pwm_FtmInternal_CalculatePhaseShiftCnVReg
(
    VAR         (Pwm_Ftm_ModuleType,        AUTOMATIC)                      nHwModuleIdx,
    VAR         (uint16,                    AUTOMATIC)                      u16DutyCycle,
...
(
    ...
    VAR(Pwm_DataUpdateType,   AUTOMATIC)   eSyncUpdate,
#endif
    VAR(Pwm_AlignmentType,   AUTOMATIC)   eAlignment
);
...
*           calcualted value in the Pwm_Ftm_au16ModRegValue[] and Pwm_Ftm_au16Period[] array
*
* @param[in]    nFtmModule                   Ftm module Id
* @param[in]    nPeriod                      Period value
* @param[in]    eAlignement                  Edge alignment type
*/
static FUNC(void, PWM_CODE) Pwm_FtmInternal_Calculate
(
        VAR(Pwm_Ftm_ModuleType, AUTOMATIC)   nFtmModule,
...
* @param[in]    eAlignement               Edge alignment type
* @param[in]    bActivationState          Activation state of the Sw output control 
*               (true - means sw control is enabled, false - sw control disabled for the given channel)
*/
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief        This inline function update period ticks in the end of each duty cycle
* @details  
*
* @param[in]    u8ModuleIdShift      FTM Module need to be shifted
*/
static FUNC (void, PWM_CODE) Pwm_Ftm_ChangeModValue 
(
    CONST         (uint8,                     AUTOMATIC)                 u8ModuleIdShift
...
)
{
    ...
    VAR         (uint16,                    AUTOMATIC)                      u16CnVRegVal;

    /* If duty is 100% then increase by 1 the value so it will exceed value of period*/
    if (((uint16)0x8000U) == u16DutyCycle)
    {
        u16CnVRegVal = Pwm_Ftm_au16ModRegValue[nHwModuleIdx] + 1U;

        /* Avoid compiler warning */
        (void)eAlignment;
    }
    /* If duty cycle is 0%, CnV = 0 */
    else if (((uint16)0U) == u16DutyCycle)
    {
        u16CnVRegVal = (uint16)0U;

        /* Avoid compiler warning */
        (void)eAlignment;
    }
    /* If duty cycle is between 0% and 100% */
    else
    {
        /* For center-aligned, Duty cycle = (2 * (CnV - CNTIN)) / (2 * (MOD - CNTIN)).
           As CNTIN = 0, Period = 2 * MOD, then CnV = (Period / 2) * Duty cycle */
        if (PWM_CENTER_ALIGNED == eAlignment)
        {
            u16CnVRegVal = (uint16)((((uint32)Pwm_Ftm_au16Period[nHwModuleIdx] * (uint32)u16DutyCycle) >> ((uint32)15U)) >> (uint32)1U);
        }
        /* For all the rest of PWM edges, Duty cycle = (CnV - CNTIN) / (MOD - CNTIN + 1).
           As CNTIN = 0, Period = MOD + 1, then CnV = Period * Duty cycle. */
        else
        {
            u16CnVRegVal = (uint16)(((uint32)Pwm_Ftm_au16Period[nHwModuleIdx] * (uint32)u16DutyCycle) >> 15U);
        }
    }
...
)
{
    ...
    {
        u16CnVRegVal = u16PhaseShift;
    }
    else
    {
        /* The n+1 match can occur at the next cycle */
        u16CnVRegVal = (u16PhaseShift + (uint16)(((uint32)Pwm_Ftm_au16Period[nHwModuleIdx] * (uint32)u16DutyCycle) >> 15U))%(Pwm_Ftm_au16Period[nHwModuleIdx]);
    }
...
)
{
    ...
    if((PWM_COMBINE_COMPL == eAlignment) || (PWM_COMBINE_SYNCED == eAlignment))
    {
        ...
        u16RegDutyVal = Pwm_FtmInternal_CalculateCnVReg(nFtmModule, u16Duty, eAlignment);
    ...
    {
        /* Scale duty cycle to the current period */
        u16RegDutyVal = Pwm_FtmInternal_CalculatePhaseShiftCnVReg(nFtmModule, u16Duty, u16PhaseShiftValue);
...
)
{
    ...
    u16TempPolVal = (uint16)((uint32)u16TempPolVal << PWM_FTM_SWOCTRL_BITS_OFFSET_U16);

    /* Clear the SWOCTRL bit(s) from the SwCtrlValue for the given channel */
     u16SwoCtrlValue = u16SwoCtrlValue & ((uint16)(~(((uint32)u16ChanBitMask) << (((uint16)nFtmChannel) + PWM_FTM_SWOCTRL_BITS_OFFSET_U16))));
    ...
    if ((boolean)TRUE == bActivationState)
    {
        if (PWM_HIGH == nFtmOutputLevel)
        {
            ...
            u16SwoCtrlBitMask =  ((uint16)(((uint32)u16ChanBitMask) << (((uint16)nFtmChannel) + PWM_FTM_SWOCTRL_BITS_OFFSET_U16)));
        }
        else
        {
            /* Do nothing*/
...
)
{
    if (PWM_CENTER_ALIGNED == eAlignment)
    {
        ...
        Pwm_Ftm_au16ModRegValue[nFtmModule] = (uint16)((uint16)nPeriod  >> (uint16)1U);
    ...
    Pwm_Ftm_au16Period[nFtmModule] = (uint16)nPeriod;
 }
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief        This inline function update period 
* @details  
*
* @param[in]    u8ModuleIdShift         Ftm module need to be shifted
*/
...
)
{
    ...
    if ( PWM_FTM_FIRST_INT == Pwm_Ftm_u8SetPhaseShiftCheck) /* update MOD register at the first interrupt */
    {
       
        REG_WRITE32(FTM_MOD_ADDR32(u8ModuleIdShift), Pwm_Ftm_u16PhaseShift_TimeBase);     /* change MOD = Pwm_Ftm_u16PhaseShift_TimeBase*/
        REG_BIT_SET32(FTM_PWMLOAD_ADDR32(u8ModuleIdShift), FTM_PWMLOAD_LDOK_MASK_U32);       /* update MOD register from buffer at reload point */
      
    }
    else if ( PWM_FTM_SECOND_INT == Pwm_Ftm_u8SetPhaseShiftCheck) /* update MOD register at the second interrupt*/
    {

        REG_WRITE32(FTM_MOD_ADDR32(u8ModuleIdShift), Pwm_Ftm_au16ModRegValue[u8ModuleIdShift] );  /* change MOD value to original MOD value */
        REG_BIT_SET32(FTM_PWMLOAD_ADDR32(u8ModuleIdShift), FTM_PWMLOAD_LDOK_MASK_U32);            /* update MOD register from buffer at reload point */
        REG_BIT_CLEAR32(FTM_SC_ADDR32(u8ModuleIdShift), FTM_SC_TOIE_MASK_U32);                    /* clear the enable flag interrupt */
    }
    else
    {
        /*Do nothing */
    ...
    if ((PWM_FTM_FIRST_INT == Pwm_Ftm_u8SetPhaseShiftCheck || PWM_FTM_SECOND_INT == Pwm_Ftm_u8SetPhaseShiftCheck))
    {
        Pwm_Ftm_u8SetPhaseShiftCheck ++;
    }
}
#endif
    ...
    for (u8ModuleId = ((uint8)0U);  u8ModuleId < u8NumModules;  u8ModuleId++)
    {
        ...
        for (u8CurChannel = ((uint8)0U); u8CurChannel < u8NumChannels; u8CurChannel++)
        {
            ...
            if (u8InternalModuleId == u8HwModuleId)
            {
...
#if ((PWM_NOTIFICATION_SUPPORTED == STD_ON) || (PWM_FORCE_OUTPUT_TO_ZERO_API == STD_ON ))
                /* Store polarity of current channel in a global array */
                Pwm_Ftm_eChannelPolarity[u8CurChannel] = pFtmChannelConfig->ePwmPolarity;
#endif

#if ((PWM_SET_OUTPUT_TO_IDLE_API == STD_ON) || (PWM_DE_INIT_API == STD_ON))
                /* Store polarity of current channel in a global array */
                Pwm_Ftm_eChannelIdleState[u8CurChannel] = pFtmChannelConfig->ePwmIdleState;
#endif
                ...
                Pwm_FtmInternal_Calculate(u8HwModuleId, u16RegPeriodVal, eAlignment);
        ...
        /*@violates @ref Pwm_Ftm_C_REF_2 cast from unsigned int to pointer */
        /*@violates @ref Pwm_Ftm_C_REF_9 cast from unsigned int to pointer */
        REG_WRITE32(FTM_MOD_ADDR32(u8HwModuleId), (uint32)(Pwm_Ftm_au16ModRegValue[u8HwModuleId]));
...
)
{
    ...
    for (u8CurChannel = ((uint8)0U); u8CurChannel < u8NumChannels; u8CurChannel++)
    {
        ...
        eIdleState = Pwm_Ftm_eChannelIdleState[u8CurChannel];
...
)
{
    ...
    Pwm_FtmInternal_Calculate(u8ModuleId, nPeriod, eAlignment);

    /*write FTMn_MOD register*/
    /*@violates @ref Pwm_Ftm_C_REF_2 cast from unsigned int to pointer */
    /*@violates @ref Pwm_Ftm_C_REF_9 cast from unsigned int to pointer */
    REG_WRITE32(FTM_MOD_ADDR32(u8ModuleId), (uint32)(Pwm_Ftm_au16ModRegValue[u8ModuleId]));
...
)
{
    ...
    eIdleState = Pwm_Ftm_eChannelIdleState[u8IdxChannelConfig];
...
)
{
    ...
    ePolarity = Pwm_Ftm_eChannelPolarity[u8IdxChannelConfig];
...
)
{
    ...
    else
    {
        ...
        if ((boolean)FALSE == bForce)
        {
            ...
            {
                /* Get channel idle state */
                nFtmOutputLevel = Pwm_Ftm_eChannelIdleState[u8IdxChannelConfig];
...
)
{
    ...
    Pwm_Ftm_ChangeModValue (u8FtmModule);
...
#endif /* (((PWM_NOTIFICATION_SUPPORTED == STD_ON) || (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)) */
...
)
{
    ...
    if (((uint32) 0) != (u32FaultStatus & FTM_FMS_FAULTF_FAULT_U32))
    {
        ...
        if ((((uint8) 0U) != nFaultCheckEnable) && (((uint8) 0U) == (Pwm_Ftm_u8FaultCheck & (uint8)((uint32) 1U << u8ModuleId))))
        {
            ...
            /*@violates @ref Pwm_Ftm_C_REF_2 cast from unsigned int to pointer */
            /*@violates @ref Pwm_Ftm_C_REF_9 cast from unsigned int to pointer */
            u32TempRegVal = (REG_READ32(FTM_SWOCTRL_ADDR32(u8ModuleId)) & \
                             (FTM_SWOCTRL_CH7OC_MASK_U32|FTM_SWOCTRL_CH6OC_MASK_U32|FTM_SWOCTRL_CH5OC_MASK_U32|FTM_SWOCTRL_CH4OC_MASK_U32|FTM_SWOCTRL_CH3OC_MASK_U32|FTM_SWOCTRL_CH2OC_MASK_U32|FTM_SWOCTRL_CH1OC_MASK_U32|FTM_SWOCTRL_CH0OC_MASK_U32) \
                            );
...
)
{
    ...
    if(0U == u16Period)
    {
        ...
        Pwm_Ftm_au16ModRegValue[u8ModuleId] = u16Period;
    } 
    else
    {
        Pwm_Ftm_au16Period[u8ModuleId] = u16Period;
        Pwm_Ftm_au16ModRegValue[u8ModuleId] = (u16Period - ((uint16)1U));
    }
    ...
    u16RegShiftVal = (uint16)(((uint32)Pwm_Ftm_au16Period[u8ModuleId] * (uint32)u16PhaseShift) >> ((uint32)15U));
...
#endif /*#if (PWM_ENABLE_TRIGGER_API == STD_ON)*/
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief        This function set phase shift value between two FTM instances
*
* @details      In order to have phase shifted between 2 FTM instances based on global time base (GTB) synchronization, Pwm_Ftm_SetPhaseShift_TimeBase is introduced.
*               This function bases on global time base (GTB) feature and the interrupt
*               With the case use derivative only having two FTM instances but need to control more independent hardware channels.
*
* @param[in]    u8ModuleIdShift         FTM module ID will be shifted
* @param[in]    u16PhaseShift           Phase shift value will be shifted
*
* @return       void
*
*/
FUNC (void, PWM_CODE) Pwm_Ftm_SetPhaseShift_TimeBase
(
    VAR         (uint8,                     AUTOMATIC)                 u8ModuleIdShift,
    VAR         (uint16,                    AUTOMATIC)                 u16PhaseShift
...
)
{
    ...
    /* Update the value need to be shifted into the global variable */
    Pwm_Ftm_u16PhaseShift_TimeBase = u16PhaseShift - 1;
    /* Update the value to check interrupt*/
    Pwm_Ftm_u8SetPhaseShiftCheck = 0;
    
    /* Disable TOIE interrupt */
    /*@violates @ref Pwm_Ftm_C_REF_2 cast from unsigned int to pointer */
    /*@violates @ref Pwm_Ftm_C_REF_9 cast from unsigned int to pointer */
    REG_BIT_CLEAR32(FTM_SC_ADDR32(u8ModuleIdShift), FTM_SC_TOIE_MASK_U32);
    
    /* Clear TOF interrupt flags */
    /*@violates @ref Pwm_Ftm_C_REF_2 cast from unsigned int to pointer */
    /*@violates @ref Pwm_Ftm_C_REF_9 cast from unsigned int to pointer */
    REG_BIT_CLEAR32(FTM_SC_ADDR32(u8ModuleIdShift), FTM_SC_TOF_MASK_U32);
    
    /* Enable TOIE interrupt */
    /*@violates @ref Pwm_Ftm_C_REF_2 cast from unsigned int to pointer */
    /*@violates @ref Pwm_Ftm_C_REF_9 cast from unsigned int to pointer */
    REG_BIT_SET32(FTM_SC_ADDR32(u8ModuleIdShift), FTM_SC_TOIE_MASK_U32);
    
    SchM_Exit_Pwm_PWM_EXCLUSIVE_AREA_52();

}
#endif /* PWM_SET_PHASE_SHIFT_TIMEBASE_API */
...
#endif /* (PWM_SET_PHASE_SHIFT_API == STD_ON) || (PWM_SET_PHASE_SHIFT_NO_UPDATE_API == STD_ON) */
...
#endif /* (PWM_SET_PERIOD_AND_DUTY_NO_UPDATE_API == STD_ON) || (PWM_SET_DUTY_CYCLE_NO_UPDATE_API == STD_ON) */
...
#endif /* (PWM_DISABLE_TRIGGER_API == STD_ON) || (PWM_ENABLE_TRIGGER_API == STD_ON) */
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief            This function will check module id that will be used
* @details
*
* @param[in]        u8FtmModuleId    Module id will be used
* @param[in]        pFtmIpConfig     Pointer to Ftm channels configuration structure
*
* @retVal           Std_ReturnType
*/
FUNC (Std_ReturnType, PWM_CODE) Pwm_Ftm_ValidateModuleUsed
(
    VAR         (uint8,                   AUTOMATIC)                   u8FtmModuleId,
    P2CONST     (Pwm_Ftm_IpConfigType,    AUTOMATIC, PWM_APPL_CONST)   pFtmIpConfig
)
{
...
* @retVal           Std_ReturnType
*/
FUNC (Std_ReturnType, PWM_CODE) Pwm_Ftm_ValidateSetPhaseShiftTimeBase
(
    VAR         (uint8,                   AUTOMATIC)                          u8FtmModuleIdOrigin,
    VAR         (uint8,                   AUTOMATIC)                          u8FtmModuleIdShift,
    VAR         (uint16,                  AUTOMATIC)                          u16FtmPhaseShift,
    P2CONST     (Pwm_Ftm_IpConfigType,    AUTOMATIC, PWM_APPL_CONST)          pFtmIpConfig
)
{
    /* Variable to store the value returned by the function */
    VAR(Std_ReturnType, AUTOMATIC) RetVal = (Std_ReturnType)E_OK;
    /* Default period value */
    VAR(uint16,         AUTOMATIC) u16PeriodOrigin;
    /* Default period value */
    VAR(uint16,         AUTOMATIC) u16PeriodShift;
    /* Id of logic Ftm Module this channel belongs to */
    VAR(Pwm_Ftm_ModuleType,          AUTOMATIC)                       u8ModuleId;
    /* Id of hardware Ftm Module this channel belongs to */
    VAR    (Pwm_Ftm_ModuleType,          AUTOMATIC)                   u8HwModuleId;
    ...
    for (u8ModuleId = (uint8)0U;  u8ModuleId < (pFtmIpConfig->nNumModules);  u8ModuleId++)
    {
        ...
        if (u8FtmModuleIdOrigin == u8HwModuleId)
        {
            /* Get period value for FTM module */
            u16PeriodOrigin = Pwm_Ftm_au16Period[u8FtmModuleIdOrigin];
        }

        if (u8FtmModuleIdShift == u8HwModuleId)
        {
            /* Get period value for FTM module */
            u16PeriodShift = Pwm_Ftm_au16Period[u8FtmModuleIdShift];
        }
    }
    
    if (u16PeriodShift != u16PeriodOrigin)
    {
        RetVal = (Std_ReturnType)E_NOT_OK;
    }
    else
    {
        if (u16PeriodShift < u16FtmPhaseShift)
        {
            RetVal = (Std_ReturnType)E_NOT_OK;
        }
    }
...
#endif /* (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON) */

...
#define PWM_SW_PATCH_VERSION_C                  2
/**@}*/
...
LOCAL_INLINE  FUNC(Std_ReturnType, PWM_CODE) Pwm_ValidateModuleUsed
(
    VAR(uint8,               AUTOMATIC) u8ModuleId,
    VAR(uint8,               AUTOMATIC) u8ServiceId
);
...
#if ((PWM_SET_CHANNEL_OUTPUT_API == STD_ON) && (PWM_OUTPUT_STATE_PLAUSABILITY == STD_ON))
LOCAL_INLINE  FUNC(Std_ReturnType, PWM_CODE) Pwm_ValidateOutputState
(
    VAR(Pwm_ChannelType,      AUTOMATIC) ChannelNumber,
    VAR(Pwm_StateType,        AUTOMATIC) nState
);
#endif /* (PWM_SET_CHANNEL_OUTPUT_API == STD_ON) && (PWM_OUTPUT_STATE_PLAUSABILITY == STD_ON) */
...
#if ((PWM_SET_OUTPUT_TO_IDLE_API == STD_ON) && (PWM_SETOUTPUTTOIDLE_PLAUSABILITY == STD_ON))
LOCAL_INLINE FUNC(Std_ReturnType, PWM_CODE) Pwm_ValidateSetOutPutToIdle
(
    VAR(Pwm_ChannelType,          AUTOMATIC) ChannelNumber
);
#endif /* (PWM_SET_OUTPUT_TO_IDLE_API == STD_ON) && (PWM_SETOUTPUTTOIDLE_PLAUSABILITY == STD_ON) */
/*===============================================================================================*/

#ifdef PWM_SET_PHASE_SHIFT_TIMEBASE_API
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
LOCAL_INLINE  FUNC(Std_ReturnType, PWM_CODE) Pwm_ValidateParamsPhaseShiftTimeBase
(
...
)
{
    ...
    VAR(uint32, AUTOMATIC) u32PeriodMaxValue =  (uint32)0U;
#else
    (void) Period;
    (void) ChannelNumber;
#endif
...
#if (PWM_MAX_PERIOD_PLAUSABILITY == STD_ON)
        u32PeriodMaxValue = Pwm_Ipw_GetMaxPeriodValue(ChannelNumber, &Pwm_pConfig->IpConfig);
        /* @violates @ref Pwm_C_REF_8 booleand operations should not have invariant results */
        if ((uint32)Period > u32PeriodMaxValue)
        {
            Det_ReportError
            (
                (uint16) PWM_MODULE_ID,
...
)
{
   ...
   /* Variable to store the value returned by the function */
    VAR(Std_ReturnType, AUTOMATIC) nRetVal = (Std_ReturnType)E_OK;
    VAR(Pwm_NotifyType, AUTOMATIC) pfPwmChannelNotification;

#if (PWM_NOTIFICATON_PLAUSABILITY == STD_OFF)
    (void)Notification;
#endif
    pfPwmChannelNotification = (*Pwm_pConfig->pPwmChannelsConfig)[ChannelNumber].pfPwmChannelNotification;
    /* Check if Notification param is not NULL */
    if(NULL_PTR == pfPwmChannelNotification)
    {
        Det_ReportError
        (
            (uint16) PWM_MODULE_ID,
...
)
{
    ...
    retVal = Pwm_Ipw_ValidateModuleUsed(u8ModuleId, &Pwm_pConfig->IpConfig);
...
#if (PWM_SETOUTPUTTOIDLE_PLAUSABILITY == STD_OFF)
    (void)ChannelNumber;
#endif

#if (PWM_SETOUTPUTTOIDLE_PLAUSABILITY == STD_ON)
...
#endif  /* PWM_SETOUTPUTTOIDLE_PLAUSABILITY */
    return nRetVal;
}
#endif /* (PWM_SET_OUTPUT_TO_IDLE_API == STD_ON) && (PWM_SETOUTPUTTOIDLE_PLAUSABILITY == STD_ON) */
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief            Pwm_ValidateParamsPhaseShiftTimeBase
* @details          This function will check phase shift parameter and period (between 2 FTM instance) to make sure the parameter is valid.
*
...
* @param[in]        u8ServiceId        Id of the service calling this function
*
* @return           Std_ReturnType     Validity of parameters that will be used in the API
...
)
{
    ...
    retVal = Pwm_Ipw_ValidateParamsPhaseShiftTimeBase(u8ModuleIdOrigin, u8ModuleIdShift, u16PhaseShift, &Pwm_pConfig->IpConfig);
...
)
{
    ...
    if((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(ChannelNumber, PWM_SETCHANNELOUTPUT_ID))
    {
        ...
        {
#endif /* PWM_OUTPUT_STATE_PLAUSABILITY */
#endif /* (PWM_PARAM_CHECK == STD_ON) */
...
*
* @details      In order to have phase shifted between 2 FTM instances based on global time base (GTB) synchronization, Pwm_SetPhaseShift_TimeBase is introduced.
...
*
* @param[in]    u8ModuleIdOrigin        FTM module ID that will be FTM instance standard
* @param[in]    u8ModuleIdShift         FTM module ID that need to shift
...
)
{
...
#if (PWM_VALIDATE_GLOBAL_CONFIG_CALL == STD_ON)
    CallIsValid = Pwm_ValidateGlobalConfigCall(PWM_SETPHASESHIFTTIMEBASE_ID);
    if((Std_ReturnType)E_OK == CallIsValid)
    {
#endif /* PWM_VALIDATE_GLOBAL_CONFIG_CALL */

#if (PWM_PARAM_CHECK == STD_ON)
        if (((Std_ReturnType)E_OK == Pwm_ValidateModuleUsed((uint8)u8ModuleIdOrigin, PWM_SETPHASESHIFTTIMEBASE_ID)) && \
            ((Std_ReturnType)E_OK == Pwm_ValidateModuleUsed((uint8)u8ModuleIdShift, PWM_SETPHASESHIFTTIMEBASE_ID)))
        { 
            if((Std_ReturnType)E_OK == Pwm_ValidateParamsPhaseShiftTimeBase(u8ModuleIdOrigin, u8ModuleIdShift, u16PhaseShift, PWM_SETPHASESHIFTTIMEBASE_ID))
            {
#endif /* (PWM_PARAM_CHECK == STD_ON) */
                Pwm_Ipw_SetPhaseShift_TimeBase(u8ModuleIdShift, u16PhaseShift);
#if (PWM_PARAM_CHECK == STD_ON)
            }
        }
#endif /* (PWM_PARAM_CHECK == STD_ON) */
    ...
    * one side efect however executed, or b)cause control flow to change.
    */
    Pwm_EndValidateGlobalConfigCall(CallIsValid, PWM_SETPHASESHIFTTIMEBASE_ID);
#endif /* PWM_VALIDATE_GLOBAL_CONFIG_CALL */
}
#endif
#endif /* PWM_SET_PHASE_SHIFT_TIMEBASE_API */

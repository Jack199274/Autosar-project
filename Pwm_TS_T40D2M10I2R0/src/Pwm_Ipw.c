...
#endif /* (PWM_FAULT_SUPPORTED == STD_ON) && (PWM_FTM_USED == STD_ON) */
...
{
    VAR(uint16, AUTOMATIC) u16DutyCycle = (uint16)0x00;

    if (PWM_FTM_CHANNEL == (*pIpConfig->pIpChannelsConfig)[nChannelNumber].nChannelIp)
    {
#if (PWM_FTM_USED == STD_ON)
        /* Call Ftm IP function */
        u16DutyCycle = Pwm_Ftm_GetChannelState((*pIpConfig->pIpChannelsConfig)[nChannelNumber].u8IdxChannelConfig, pIpConfig->pFtmIpConfig);
#endif
    }
    else if (PWM_FLEXIO_CHANNEL == (*pIpConfig->pIpChannelsConfig)[nChannelNumber].nChannelIp )
    {
#if (PWM_FLEXIO_USED == STD_ON)
        /* Call FlexIO IP function */
        u16DutyCycle = Pwm_FlexIO_GetChannelState((*pIpConfig->pIpChannelsConfig)[nChannelNumber].u8IdxChannelConfig, pIpConfig->pFlexIOIpConfig);
#endif
    }
    else
    {
...
#endif /* (PWM_FORCE_OUTPUT_TO_ZERO_API == STD_ON) */
...
#endif /* (PWM_SET_CLOCK_MODE_API == STD_ON) */
...
#endif /* (PWM_SET_PHASE_SHIFT_API == STD_ON) */
...
/**
* @brief        This function will set phase shift between two FTM instances
* @details
*      
*
* @param[in]    u8ModuleId     FTM module will be shifted 
*               Corresponding  0 -> FTM_0,
*                              1 -> FTM_1, 
*                              2 -> FTM_2...
* @param[in]    u16PhaseShift  The value will be used to shift between 2 FTM instances 
*
* @return       void
*
*/
FUNC (void, PWM_CODE) Pwm_Ipw_SetPhaseShift_TimeBase
(
...
#if (PWM_FTM_USED == STD_ON)
        /* Call Ftm IP function */
        Pwm_Ftm_SetPhaseShift_TimeBase(u8ModuleIdShift, u16PhaseShift);
#else
    (void) u8ModuleIdShift;
    (void) u16PhaseShift;
#endif
...
#if (PWM_SET_PHASE_SHIFT_TIMEBASE_API == STD_ON)
/**
* @brief            Pwm_Ipw_ValidateParamsPhaseShiftTimeBase
* @details          This function check valid of the parameter that will be used.
*
* @param[in]        u8ModuleIdOrigin   Module id will be standard
* @param[in]        u8ModuleIdShift    Module id will be shifted
* @param[in]        u16PhaseShift      The value will be shifted
* @param[in]        pIpConfig          Pointer to PWM top configuration structure
*
* @return           Std_ReturnType     Validity of parameter that will be used in the API
* @retval           E_OK               The parameter id is valid
* @retval           E_NOT_OK           The parameter id is not valid
*/
FUNC (Std_ReturnType, PWM_CODE) Pwm_Ipw_ValidateParamsPhaseShiftTimeBase
/* @violates @ref Pwm_IPW_C_REF_5 Identifier exceeds 31 chars. */
(
    VAR(uint8,                     AUTOMATIC)                 u8ModuleIdOrigin,
    VAR(uint8,                     AUTOMATIC)                 u8ModuleIdShift,
    VAR(uint16,                    AUTOMATIC)                 u16PhaseShift,
    CONSTP2CONST(Pwm_IpConfigType, AUTOMATIC, PWM_APPL_CONST) pIpConfig
)
{
    ...
    VAR(Std_ReturnType, AUTOMATIC) RetVal = (Std_ReturnType)E_OK;
#if (PWM_FTM_USED == STD_ON)
    RetVal = Pwm_Ftm_ValidateSetPhaseShiftTimeBase(u8ModuleIdOrigin, u8ModuleIdShift, u16PhaseShift, pIpConfig->pFtmIpConfig);   
#else
    (void)u8ModuleIdOrigin;
    (void)u8ModuleIdShift;
    (void)u16PhaseShift;
    (void)pIpConfig;
...
*                 In order to be valid, the module id should be used in the configuration.
*
* @param[in]      u8ModuleId      The Id of the ipv module
* @param[in]      pIpConfig       Pointer to PWM top configuration structure
*
* @return         Std_ReturnType  Validity of hardware module that will be used in the API
...
* @note           .
*/
FUNC (Std_ReturnType, PWM_CODE) Pwm_Ipw_ValidateModuleUsed
(
    VAR         (uint8           , AUTOMATIC)                 u8ModuleId,
    CONSTP2CONST(Pwm_IpConfigType, AUTOMATIC, PWM_APPL_CONST) pIpConfig
)
...
#if (PWM_FTM_USED == STD_ON)
    RetVal = Pwm_Ftm_ValidateModuleUsed(u8ModuleId, pIpConfig->pFtmIpConfig);
#else
    (void)u8ModuleId;
    (void)pIpConfig;
#endif

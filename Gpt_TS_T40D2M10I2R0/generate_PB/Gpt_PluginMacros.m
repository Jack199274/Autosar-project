...
[!MACRO "BuildConfigSet"!]
    ...
    [!IF "node:value(GptAutosarExt/GptEnableDualClockMode)"!][!//
...
[!IF "contains(GptHwChannel, 'SRTC')"!][!//
    ...
    [!ENDIF!][!//
[!ELSE!][!//
    [!CODE!][!WS "12"!](uint8)(0U),             /* SRTC Clock Select, SRTC is not USED */ [!CR!][!ENDCODE!][!//
[!ENDIF!][!//
...
[!IF "node:value(GptLPitIsExternalTrigger)"!][!//
            (uint32)([!IF "node:exists(GptLPitTriggerChannels)"!]((uint32)[!"text:split(node:value(GptLPitTriggerChannels), '_')[2]"!]U << LPIT_TCTRL_TRG_SEL_SHIFT) |[!ENDIF!] \
[!IF "node:value(GptLPitIsExternalTrigger)"!][!//
            ((uint32)0U << LPIT_TCTRL_TRG_SRC_SHIFT) | \
[!ELSE!][!//
            ((uint32)1U << LPIT_TCTRL_TRG_SRC_SHIFT) | \
[!ENDIF!][!//
[!IF "node:value(GptLPitEnReloadOnTrigger)"!][!//
            ((uint32)1U << LPIT_TCTRL_TRG_TROT_SHIFT) | \
[!ELSE!][!//
            ((uint32)0U << LPIT_TCTRL_TRG_TROT_SHIFT) | \
[!ENDIF!][!//
[!IF "node:value(GptLPitEnStopOnInterrupt)"!][!//
            ((uint32)1U << LPIT_TCTRL_TRG_TSOI_SHIFT) | \
[!ELSE!][!//
            ((uint32)0U << LPIT_TCTRL_TRG_TSOI_SHIFT) | \
[!ENDIF!][!//
[!IF "node:value(GptLPitEnStartOnTrigger)"!][!//
            ((uint32)1U << LPIT_TCTRL_TRG_TSOT_SHIFT)),
[!ELSE!][!//
            ((uint32)0U << LPIT_TCTRL_TRG_TSOT_SHIFT)),
[!ENDIF!][!//
[!ELSE!][!//
...
[!ENDCODE!][!//
[!CODE!][!//
[!CODE!]#if (GPT_CHAIN_MODE == STD_ON)[!CR!][!ENDCODE!]
    [!IF "contains(GptHwChannel, 'LPIT')"!][!//
    [!CODE!][!WS"4"!][!IF "((node:value(../../../GptAutosarExt/GptChainModeApi) = 'true') and (GptChainMode = 'true') and (contains(GptHwChannel,'LPIT') = 'true') and (num:i(substring-after(GptHwChannel, 'CH_')))!=num:i(0))"!](boolean)TRUE[!ELSE!](boolean)(FALSE)[!ENDIF!] /* Chain Mode Enable */[!CR!][!ENDCODE!]
    [!ELSE!][!//
    [!CODE!][!WS"4"!](boolean)FALSE /* Chain Mode not used for STM, EMIOS, FTM or ETIMER */[!CR!][!ENDCODE!]
[!ENDIF!][!//
